...
namespace nCine::Backends
{
	/**
		...
		Owns what every backend class needs but none should open twice: the OS library bases, the
		EClock timer (the machine's only fine-grained monotonic clock, ~709 kHz on PAL), and the
	...
	*/
	...
	public:
		...
		/** @brief Opens the OS libraries and the timer, runs the performance probe; `false` is fatal for the caller */
		...
		*/
		...
		/** @brief EClock rate in ticks per second (~709379 PAL / ~715909 NTSC), valid after @ref Initialize() */
