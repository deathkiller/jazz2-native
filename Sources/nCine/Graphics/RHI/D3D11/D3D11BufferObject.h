...
namespace nCine::RHI::D3D11
{
	...
	*/
	...
	private:
		...
		SmallVector<std::uint8_t, 0> _storage;

		// GPU buffer mirroring the host store for the vertex/index targets (uniform buffers stay host-only and
		// are forwarded as ranges). Refreshed lazily from the store when @ref _gpuDirty is set.
		mutable ID3D11Buffer* _gpuBuffer;
		mutable std::size_t _gpuBufferCapacity;
