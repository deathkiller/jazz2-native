...
namespace nCine::RHI::GX
{
	/**
		...
		block, committing is a no-op — the whole block is published by @ref GxShaderUniformBlocks.
	*/
	...
	public:
		...
		explicit GxUniformCache(const GxUniform* uniform)
			: uniform_(uniform), dataPointer_(nullptr), isDirty_(false) {}

		inline const GxUniform* GetUniform() const {
			return uniform_;
		}
	...
	/**
		...
		GxUniformCache for each member, each pointing at its member offset within that buffer. The block is
		uploaded as one contiguous range by @ref GxShaderUniformBlocks.
	*/
	...
	public:
		...
		explicit GxUniformBlockCache(GxUniformBlock* uniformBlock);

		inline const GxUniformBlock* uniformBlock() const {
		...
		GxUniformCache* GetUniform(StringView name);
		void SetBlockBinding(std::int32_t blockBinding);

	private:
		GxUniformBlock* uniformBlock_;
		std::uint8_t* dataPointer_;
