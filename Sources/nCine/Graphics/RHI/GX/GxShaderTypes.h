...
namespace nCine::RHI::GX
{
	...
	/**
		...
		Holds everything the GX backend needs about one uniform imported from the offline
		...
		value storage — a @ref GxUniformCache holds the value.
	*/
	class GxUniform
	{
		friend class GxShaderProgram;
		friend class GxUniformBlock;
	...
	public:
		...
		GxUniform(GxShaderProgram* owner, const char* name, ShaderCompiler::UniformType type, std::int32_t arraySize, std::int32_t location)
		...
		inline GxShaderProgram* GetOwner() const {
			return _owner;
		}
	...
	/**
		...
		metadata of its member uniforms (each with a std140 offset). The GX backend does not pad the
		size to any device alignment, so @ref GetAlignAmount() is always 0.
	*/
	class GxUniformBlock
	{
		friend class GxShaderProgram;
		friend class GxUniformBlockCache;
	...
	public:
		...
		GxUniformBlock(std::uint32_t index, const char* name, std::int32_t dataSize)
	...
	*/
	...
	public:
		...
		GxAttribute(const char* name, ShaderCompiler::UniformType type, std::int32_t location)
