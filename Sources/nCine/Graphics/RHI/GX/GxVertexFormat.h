...
namespace nCine::RHI::GX
{
	...
	/**
		@brief Vertex layout description of the GX backend (aliased as `RHI::VertexFormat`)
	...
	*/
	...
	public:
		...
		public:
			...
			// needing the complete `GxBuffer` type in this header
			...
			inline const GxBuffer* GetVbo() const {
				return vbo_;
			}
			...
			inline void setVbo(const GxBuffer* vbo) {
				vbo_ = vbo;
			}
		...
		inline const GxBuffer* GetIbo() const {
			return ibo_;
		}
		inline void SetIbo(const GxBuffer* ibo) {
			ibo_ = ibo;
		}
		/** @brief Applies the vertex format (no-op for the GX backend) */
		void Define() {}
	...
	/**
		@brief Vertex array object stub of the GX backend (aliased as `RHI::VertexArray`)
	...
	*/
	...
	{
	public:
		GxVertexArray() = default;
		~GxVertexArray() = default;

		GxVertexArray(const GxVertexArray&) = delete;
		GxVertexArray& operator=(const GxVertexArray&) = delete;
