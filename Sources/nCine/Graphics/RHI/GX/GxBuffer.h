...
{
	/**
		@brief Host-memory buffer object of the GX backend (aliased as `RHI::Buffer`)

		The GX backend consumes vertices and uniform blocks from host memory (GX draws are fed through the
	...
	*/
	...
	{
	public:
		explicit GxBuffer(BufferTarget target);
		~GxBuffer() = default;

		GxBuffer(const GxBuffer&) = delete;
		GxBuffer& operator=(const GxBuffer&) = delete;
		...
		/** @brief Sets a debug label for the buffer (ignored by the GX backend) */
		...
		/** @brief Records the buffer handle bound for a target (inert for the GX backend) */
		inline static void SetBoundHandle(std::uint32_t target, std::uint32_t glHandle) {
			static_cast<void>(target);
			static_cast<void>(glHandle);
		}
		/** @brief Marks a buffer handle as bound for a target (inert for the GX backend) */
