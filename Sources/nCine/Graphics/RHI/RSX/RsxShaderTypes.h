...
namespace nCine::RHI::RSX
{
	...
	/**
		...
		RsxUniformCache holds the value.
	*/
	class RsxUniform
	{
		friend class RsxShaderProgram;
		friend class RsxUniformBlock;
	...
	public:
		...
		RsxUniform(RsxShaderProgram* owner, const char* name, ShaderCompiler::UniformType type, std::int32_t arraySize, std::int32_t location)
		...
		inline RsxShaderProgram* GetOwner() const {
			return _owner;
		}
	...
	*/
	...
	{
		friend class RsxShaderProgram;
		friend class RsxUniformBlockCache;
	...
	public:
		...
		RsxUniformBlock(std::uint32_t index, const char* name, std::int32_t dataSize)
		...
		RsxUniform* GetUniform(const char* name) {
			for (RsxUniform& u : _members) {
	...
	private:
		...
		SmallVector<RsxUniform, 0> _members;
	...
	*/
	...
	public:
		...
		RsxAttribute(const char* name, ShaderCompiler::UniformType type, std::int32_t location)
