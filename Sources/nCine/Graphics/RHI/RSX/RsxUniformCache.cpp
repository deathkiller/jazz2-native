...
namespace nCine::RHI::RSX
{
	...
	bool RsxUniformCache::SetFloatValue(float v0)
	{
	...
	bool RsxUniformCache::SetFloatValue(float v0, float v1)
	{
	...
	bool RsxUniformCache::SetFloatValue(float v0, float v1, float v2)
	{
	...
	bool RsxUniformCache::SetFloatValue(float v0, float v1, float v2, float v3)
	{
	...
	bool RsxUniformCache::SetIntVector(const std::int32_t* vec)
	{
	...
	bool RsxUniformCache::SetIntValue(std::int32_t v0)
	{
	...
	bool RsxUniformCache::SetIntValue(std::int32_t v0, std::int32_t v1)
	{
	...
	bool RsxUniformCache::SetIntValue(std::int32_t v0, std::int32_t v1, std::int32_t v2)
	{
	...
	bool RsxUniformCache::SetIntValue(std::int32_t v0, std::int32_t v1, std::int32_t v2, std::int32_t v3)
	{
	...
	bool RsxUniformCache::CommitValue()
	{
	...
	bool RsxUniformCache::CheckFloat() const
	{
		return (_uniform != nullptr && _uniform->IsFloat());
	}

	bool RsxUniformCache::CheckInt() const
	{
		return (_uniform != nullptr && !_uniform->IsFloat());
	}

	bool RsxUniformCache::CheckComponents(std::uint32_t requiredComponents) const
	{
	...
	RsxUniformBlockCache::RsxUniformBlockCache(RsxUniformBlock* uniformBlock)
		: _uniformBlock(uniformBlock), _dataPointer(nullptr), _usedSize(0)
	{
		...
		for (RsxUniform& member : uniformBlock->_members) {
			_uniformCaches.push_back({member.GetName(), RsxUniformCache(&member)});
		}
	}

	std::uint32_t RsxUniformBlockCache::GetIndex() const
	{
		return (_uniformBlock != nullptr ? _uniformBlock->GetIndex() : 0);
	}

	std::int32_t RsxUniformBlockCache::GetBindingIndex() const
	{
		return (_uniformBlock != nullptr ? _uniformBlock->GetBindingIndex() : 0);
	}

	std::int32_t RsxUniformBlockCache::GetSize() const
	{
		return (_uniformBlock != nullptr ? _uniformBlock->GetSize() : 0);
	}

	std::uint8_t RsxUniformBlockCache::GetAlignAmount() const
	{
		return (_uniformBlock != nullptr ? _uniformBlock->GetAlignAmount() : 0);
	}

	void RsxUniformBlockCache::SetDataPointer(std::uint8_t* dataPointer)
	{
	...
	void RsxUniformBlockCache::SetUsedSize(std::int32_t usedSize)
	{
		if (usedSize >= 0) {
			_usedSize = usedSize;
		}
	...
	RsxUniformCache* RsxUniformBlockCache::GetUniform(StringView name)
	{
		for (NamedCache& named : _uniformCaches) {
			if (name == named.Name) {
				return &named.Cache;
	...
	void RsxUniformBlockCache::SetBlockBinding(std::int32_t blockBinding)
	{
