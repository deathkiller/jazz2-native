...
namespace nCine::RHI::PICA
{
	...
	/**
		...
		Holds everything the PICA backend needs about one uniform imported from the offline
		...
		value storage — a @ref PicaUniformCache holds the value.
	*/
	class PicaUniform
	{
		friend class PicaShaderProgram;
		friend class PicaUniformBlock;
	...
	public:
		...
		PicaUniform(PicaShaderProgram* owner, const char* name, ShaderCompiler::UniformType type, std::int32_t arraySize, std::int32_t location)
		...
		inline PicaShaderProgram* GetOwner() const {
			return _owner;
		}
	...
	/**
		...
		metadata of its member uniforms (each with a std140 offset). The PICA backend does not pad the
		size to any device alignment, so @ref GetAlignAmount() is always 0.
	*/
	class PicaUniformBlock
	{
		friend class PicaShaderProgram;
		friend class PicaUniformBlockCache;
	...
	public:
		...
		PicaUniformBlock(std::uint32_t index, const char* name, std::int32_t dataSize)
		...
		PicaUniform* GetUniform(const char* name) {
			for (PicaUniform& u : _members) {
	...
	private:
		...
		std::vector<PicaUniform> _members;
	...
	*/
	...
	public:
		...
		PicaAttribute(const char* name, ShaderCompiler::UniformType type, std::int32_t location)
