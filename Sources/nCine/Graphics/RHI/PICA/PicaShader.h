...
{
	/**
		@brief Shader-object stub of the PICA backend
	...
	*/
	...
	{
	public:
		/** @brief Compilation status of the shader (always @ref Status::Compiled for the PICA backend) */
		enum class Status
		{
			NotCompiled,
			CompilationFailed,
		...
		/** @brief When the compilation status is checked (irrelevant for the PICA backend) */
		...
		explicit PicaShader(std::uint32_t type)
			: _type(type), _status(Status::Compiled) {}
		PicaShader(std::uint32_t type, StringView filename)
			: _type(type), _status(Status::Compiled) {
