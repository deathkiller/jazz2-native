...
namespace nCine::RHI::PICA
{
	/**
		...
		block, committing is a no-op — the whole block is published by @ref PicaShaderUniformBlocks.
	*/
	...
	public:
		...
		explicit PicaUniformCache(const PicaUniform* uniform)
			: _uniform(uniform), _dataPointer(nullptr), _isDirty(false) {}

		inline const PicaUniform* GetUniform() const {
			return _uniform;
		}
	...
	/**
		...
		PicaUniformCache for each member, each pointing at its member offset within that buffer. The block is
		uploaded as one contiguous range by @ref PicaShaderUniformBlocks.
	*/
	class PicaUniformBlockCache
	{
	public:
		PicaUniformBlockCache()
			: _uniformBlock(nullptr), _dataPointer(nullptr), _usedSize(0) {}
		explicit PicaUniformBlockCache(PicaUniformBlock* uniformBlock);

		inline const PicaUniformBlock* uniformBlock() const {
		...
		PicaUniformCache* GetUniform(StringView name);
		void SetBlockBinding(std::int32_t blockBinding);

	private:
		PicaUniformBlock* _uniformBlock;
		std::uint8_t* _dataPointer;
