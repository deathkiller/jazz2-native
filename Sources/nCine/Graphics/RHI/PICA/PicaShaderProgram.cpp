...
{
	std::uint32_t PicaShaderProgram::_nextHandle = 1;

	PicaShaderProgram::PicaShaderProgram()
		: PicaShaderProgram(QueryPhase::Immediate)
	{
	}

	PicaShaderProgram::PicaShaderProgram(QueryPhase queryPhase)
	...
	PicaShaderProgram::PicaShaderProgram(StringView vertexFile, StringView fragmentFile, Introspection introspection, QueryPhase queryPhase)
		: PicaShaderProgram(queryPhase)
	{
	...
	PicaShaderProgram::PicaShaderProgram(StringView vertexFile, StringView fragmentFile, Introspection introspection)
		: PicaShaderProgram(vertexFile, fragmentFile, introspection, QueryPhase::Immediate)
	{
	}

	PicaShaderProgram::PicaShaderProgram(StringView vertexFile, StringView fragmentFile)
		: PicaShaderProgram(vertexFile, fragmentFile, Introspection::Enabled, QueryPhase::Immediate)
	{
	}
	...
	bool PicaShaderProgram::IsLinked() const
	{
		return (_status == Status::Linked || _status == Status::LinkedWithDeferredQueries || _status == Status::LinkedWithIntrospection);
	}

	bool PicaShaderProgram::AttachShaderFromFile(ShaderStage stage, StringView filename)
	{
	...
	bool PicaShaderProgram::AttachShaderFromString(ShaderStage stage, StringView string)
	{
		static_cast<void>(stage);
	...
	bool PicaShaderProgram::AttachShaderFromStrings(ShaderStage stage, ArrayView<const StringView> strings)
	{
		static_cast<void>(stage);
	...
	bool PicaShaderProgram::AttachShaderFromStringsAndFile(ShaderStage stage, ArrayView<const StringView> strings, StringView filename)
	{
		static_cast<void>(stage);
		static_cast<void>(strings);
		static_cast<void>(filename);
	...
	bool PicaShaderProgram::Link(Introspection introspection)
	{
		return FinalizeAfterLinking(introspection);
	}

	bool PicaShaderProgram::FinalizeAfterLinking(Introspection introspection)
	{
	...
	void PicaShaderProgram::Use()
	{
		PicaDevice::BindProgram(this);
	}

	void PicaShaderProgram::PerformIntrospection()
	{
		if (_introspection != Introspection::Disabled && _status != Status::LinkedWithIntrospection) {
			...
			_generatedEffect = (!_programName.empty()
				? PicaDevice::FindGeneratedEffect(_programName.data(), _variantName.data()) : nullptr);
	...
	void PicaShaderProgram::ImportReflection()
	{
		...
		for (std::size_t i = 0; i < reflection.BlockCount; i++) {
			...
			PicaUniformBlock& block = _uniformBlocks.back();
	...
	bool PicaShaderProgram::HasAttribute(const char* name) const
	{
		for (const PicaAttribute& a : _attributes) {
	...
	PicaVertexFormat::Attribute* PicaShaderProgram::GetAttribute(const char* name)
	{
		for (const PicaAttribute& a : _attributes) {
	...
	void PicaShaderProgram::DefineVertexFormat(const PicaBuffer* vbo, const PicaBuffer* ibo, std::uint32_t vboOffset)
	{
		_boundVbo = vbo;
		...
		if (vbo != nullptr) {
			for (const PicaAttribute& a : _attributes) {
				if (a.GetLocation() >= 0) {
					PicaVertexFormat::Attribute& attr = _vertexFormat[std::uint32_t(a.GetLocation())];
					attr.setVbo(vbo);
	...
	void PicaShaderProgram::SetObjectLabel(StringView label)
	{
		// The label is kept for log messages only - effect identity comes from SetProgramIdentity()
		_label = label;
	}

	void PicaShaderProgram::SetProgramIdentity(const char* programName, const char* variantName)
	{
	...
	const PicaUniformBlock* PicaShaderProgram::FindBlock(const char* name) const
	{
		for (const PicaUniformBlock& block : _uniformBlocks) {
	...
	void PicaShaderProgram::SetResolvedUniform(const char* name, const std::uint8_t* data)
	{
	...
	const std::uint8_t* PicaShaderProgram::ResolveUniform(const char* name) const
	{
