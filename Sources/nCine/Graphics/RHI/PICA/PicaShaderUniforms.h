...
namespace nCine::RHI::PICA
{
	...
	/**
		...
		Owns a @ref PicaUniformCache for every active uniform of a program that is not part of a block,
	...
	*/
	...
	{
	public:
		// The PICA backend stores the caches in a flat container whose value type is the cache
		// itself (the name lives inside each cache's uniform), so range-iteration yields `PicaUniformCache&`
		...
		using UniformHashMapType = std::vector<PicaUniformCache>;

		PicaShaderUniforms();
		explicit PicaShaderUniforms(PicaShaderProgram* shaderProgram);
		PicaShaderUniforms(PicaShaderProgram* shaderProgram, const char* includeOnly, const char* exclude);

		inline void SetProgram(PicaShaderProgram* shaderProgram) {
			SetProgram(shaderProgram, nullptr, nullptr);
		}
		void SetProgram(PicaShaderProgram* shaderProgram, const char* includeOnly, const char* exclude);
		...
		PicaUniformCache* GetUniform(const char* name);
		/** @brief Returns the container of all managed uniform caches (range-iterates as `PicaUniformCache&`) */
	...
	private:
		PicaShaderProgram* _shaderProgram;
		bool _maybeDirty;
	...
	/**
		...
		Owns a @ref PicaUniformBlockCache per active block, distributes a shared host data buffer across
	...
	*/
	...
	public:
		...
		// block), so range-iteration yields `PicaUniformBlockCache&` directly, matching the OpenGL backend.
		using UniformHashMapType = std::vector<PicaUniformBlockCache>;
		...
		explicit PicaShaderUniformBlocks(PicaShaderProgram* shaderProgram);
		PicaShaderUniformBlocks(PicaShaderProgram* shaderProgram, const char* includeOnly, const char* exclude);
		...
		PicaUniformBlockCache* GetUniformBlock(const char* name);
		/** @brief Returns the container of all managed block caches (range-iterates as `PicaUniformBlockCache&`) */
