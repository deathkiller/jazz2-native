...
#include "PicaShaderTypes.h"
#include "PicaVertexFormat.h"
#include "../RhiTypes.h"
...
namespace nCine::RHI::PICA
{
	...
	/**
		@brief Shader program of the PICA backend (aliased as `RHI::ShaderProgram`)
	...
	*/
	...
	{
		friend class PicaShaderUniforms;
		friend class PicaShaderUniformBlocks;
	...
	public:
		...
		PicaShaderProgram();
		explicit PicaShaderProgram(QueryPhase queryPhase);
		PicaShaderProgram(StringView vertexFile, StringView fragmentFile, Introspection introspection, QueryPhase queryPhase);
		PicaShaderProgram(StringView vertexFile, StringView fragmentFile, Introspection introspection);
		PicaShaderProgram(StringView vertexFile, StringView fragmentFile);
		~PicaShaderProgram();

		PicaShaderProgram(const PicaShaderProgram&) = delete;
		PicaShaderProgram& operator=(const PicaShaderProgram&) = delete;
		...
		*/
		...
		PicaVertexFormat::Attribute* GetAttribute(const char* name);

		inline void DefineVertexFormat(const PicaBuffer* vbo) {
			DefineVertexFormat(vbo, nullptr, 0);
		}
		inline void DefineVertexFormat(const PicaBuffer* vbo, const PicaBuffer* ibo) {
			DefineVertexFormat(vbo, ibo, 0);
		}
		void DefineVertexFormat(const PicaBuffer* vbo, const PicaBuffer* ibo, std::uint32_t vboOffset);
		/** @brief Returns the vertex buffer last bound by @ref DefineVertexFormat(), or `nullptr` */
		inline const PicaBuffer* GetBoundVbo() const {
			return _boundVbo;
		}
		...
		inline const PicaBuffer* GetBoundIbo() const {
			return _boundIbo;
		}
		...
		// -- PICA backend extensions (used by the device and the effects) --
		...
		*/
		struct DispatchFacts
		{
			...
			const PicaUniformBlock* InstanceBlock = nullptr;
		...
		const PicaUniformBlock* FindBlock(const char* name) const;
	...
	private:
		...
		std::vector<PicaUniform> _uniforms;
		std::vector<PicaUniformBlock> _uniformBlocks;
		std::vector<PicaAttribute> _attributes;
		...
		PicaVertexFormat _vertexFormat;
		const PicaBuffer* _boundVbo;
		std::uint32_t _boundVboOffset;
		const PicaBuffer* _boundIbo;
