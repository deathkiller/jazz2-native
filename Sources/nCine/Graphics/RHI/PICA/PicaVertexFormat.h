...
namespace nCine::RHI::PICA
{
	...
	/**
		@brief Vertex layout description of the PICA backend (aliased as `RHI::VertexFormat`)
	...
	*/
	...
	public:
		...
		public:
			...
			// needing the complete `PicaBuffer` type in this header
			...
			inline const PicaBuffer* GetVbo() const {
				return _vbo;
			}
			...
			inline void setVbo(const PicaBuffer* vbo) {
				_vbo = vbo;
			}
		...
		inline const PicaBuffer* GetIbo() const {
			return _ibo;
		}
		inline void SetIbo(const PicaBuffer* ibo) {
			_ibo = ibo;
		}
		/** @brief Applies the vertex format (no-op for the PICA backend) */
		void Define() {}
		...
		bool operator==(const PicaVertexFormat& other) const {
	...
	private:
		...
		const PicaBuffer* _ibo;
	};

	/**
		@brief Vertex array object stub of the PICA backend (aliased as `RHI::VertexArray`)
	...
	*/
	...
	{
	public:
		PicaVertexArray() = default;
		~PicaVertexArray() = default;

		PicaVertexArray(const PicaVertexArray&) = delete;
		PicaVertexArray& operator=(const PicaVertexArray&) = delete;
