...
{
	/**
		@brief Host-memory buffer object of the PICA backend (aliased as `RHI::Buffer`)

		The PICA backend consumes vertices and uniform blocks from host memory (the GPU draws are fed through a
	...
	*/
	...
	{
	public:
		explicit PicaBuffer(BufferTarget target);
		~PicaBuffer() = default;

		PicaBuffer(const PicaBuffer&) = delete;
		PicaBuffer& operator=(const PicaBuffer&) = delete;
		...
		/** @brief Sets a debug label for the buffer (ignored by the PICA backend) */
		...
		/** @brief Records the buffer handle bound for a target (inert for the PICA backend) */
		inline static void SetBoundHandle(std::uint32_t target, std::uint32_t glHandle) {
			static_cast<void>(target);
			static_cast<void>(glHandle);
		}
		/** @brief Marks a buffer handle as bound for a target (inert for the PICA backend) */
