...
namespace nCine::RHI::PICA
{
	...
	/**
		@brief Renderbuffer stub of the PICA backend (aliased as `RHI::Renderbuffer`)

		The PICA backend renders 2D only, so depth/stencil renderbuffers carry no storage; the class
		just records the format and size to satisfy the contract alias.
	*/
	class PicaRenderbuffer
	{
	public:
		PicaRenderbuffer() = default;
	...
	/**
		@brief Framebuffer stub of the PICA backend (aliased as `RHI::Framebuffer`)

		Provided only for the contract alias (the default-framebuffer rebinding some window backends use).
		The PICA backend routes off-screen rendering through @ref PicaRenderTarget instead.
	*/
	class PicaFramebuffer
	{
	public:
		PicaFramebuffer() = default;
		inline std::uint32_t GetGLHandle() const {
			return 0;
		}
		bool Bind() const {
	...
	/**
		@brief Off-screen render target of the PICA backend (aliased as `RHI::RenderTarget`)
	...
	*/
	...
	public:
		...
		PicaRenderTarget(const PicaRenderTarget&) = delete;
		PicaRenderTarget& operator=(const PicaRenderTarget&) = delete;

		/** @brief Attaches a texture as the color attachment with the given index */
		void AttachColorTexture(PicaTexture& texture, std::uint32_t index);
		/** @brief Detaches any texture from the color attachment with the given index */
		void DetachColorTexture(std::uint32_t index);

		/** @brief Records a depth/stencil buffer (no storage — the PICA backend is 2D) */
		...
		inline PicaTexture* GetColorTexture(std::uint32_t index) const {
			return (index < MaxColorAttachments ? _colorTextures[index] : nullptr);
		}

	private:
		PicaTexture* _colorTextures[MaxColorAttachments];
