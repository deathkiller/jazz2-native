...
{
	std::uint32_t PicaBuffer::_nextHandle = 1;

	PicaBuffer::PicaBuffer(BufferTarget target)
		: _handle(_nextHandle++), _target(target)
	{
	}

	bool PicaBuffer::Bind() const
	{
	...
	bool PicaBuffer::Unbind() const
	{
		return true;
	}

	void PicaBuffer::BufferData(std::size_t size, const void* data, BufferUsage usage)
	{
	...
	void PicaBuffer::BufferSubData(std::size_t offset, std::size_t size, const void* data)
	{
	...
	void PicaBuffer::BufferStorage(std::size_t size, const void* data, MapFlags flags)
	{
	...
	void PicaBuffer::BindBufferBase(std::uint32_t index)
	{
		BindBufferRange(index, 0, _storage.size());
	}

	void PicaBuffer::BindBufferRange(std::uint32_t index, std::size_t offset, std::size_t size)
	{
		...
		PicaDevice::BindUniformRange(index, _storage.data() + offset, std::uint32_t(size));
	}

	void* PicaBuffer::MapBufferRange(std::size_t offset, std::size_t length, MapFlags access)
	{
	...
	void PicaBuffer::FlushMappedBufferRange(std::size_t offset, std::size_t length)
	{
		static_cast<void>(offset);
		static_cast<void>(length);
	}

	bool PicaBuffer::Unmap()
	{
		return true;
	}

	void PicaBuffer::SetObjectLabel(StringView label)
	{
