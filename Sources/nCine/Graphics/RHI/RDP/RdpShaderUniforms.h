...
namespace nCine::RHI::RDP
{
	...
	/**
		...
		Owns a @ref RdpUniformCache for every active uniform of a program that is not part of a block,
	...
	*/
	...
	{
	public:
		// The RDP backend stores the caches in a flat container whose value type is the cache
		// itself (the name lives inside each cache's uniform), so range-iteration yields `RdpUniformCache&`
		...
		using UniformHashMapType = std::vector<RdpUniformCache>;

		RdpShaderUniforms();
		explicit RdpShaderUniforms(RdpShaderProgram* shaderProgram);
		RdpShaderUniforms(RdpShaderProgram* shaderProgram, const char* includeOnly, const char* exclude);

		inline void SetProgram(RdpShaderProgram* shaderProgram) {
			SetProgram(shaderProgram, nullptr, nullptr);
		}
		void SetProgram(RdpShaderProgram* shaderProgram, const char* includeOnly, const char* exclude);
		...
		RdpUniformCache* GetUniform(const char* name);
		/** @brief Returns the container of all managed uniform caches (range-iterates as `RdpUniformCache&`) */
	...
	private:
		RdpShaderProgram* _shaderProgram;
		bool _maybeDirty;

		UniformHashMapType _uniformCaches;
		// Name fingerprints of _uniformCaches, in the same order (see RHI::HashUniformName)
	...
	/**
		...
		Owns a @ref RdpUniformBlockCache per active block, distributes a shared host data buffer across
	...
	*/
	...
	public:
		...
		// block), so range-iteration yields `RdpUniformBlockCache&` directly, matching the OpenGL backend.
		using UniformHashMapType = std::vector<RdpUniformBlockCache>;
		...
		explicit RdpShaderUniformBlocks(RdpShaderProgram* shaderProgram);
		RdpShaderUniformBlocks(RdpShaderProgram* shaderProgram, const char* includeOnly, const char* exclude);
		...
		RdpUniformBlockCache* GetUniformBlock(const char* name);
		/** @brief Returns the container of all managed block caches (range-iterates as `RdpUniformBlockCache&`) */
