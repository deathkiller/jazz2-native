...
{
	std::uint32_t RdpShaderProgram::_nextHandle = 1;

	RdpShaderProgram::RdpShaderProgram()
		: RdpShaderProgram(QueryPhase::Immediate)
	{
	}

	RdpShaderProgram::RdpShaderProgram(QueryPhase queryPhase)
	...
	RdpShaderProgram::RdpShaderProgram(StringView vertexFile, StringView fragmentFile, Introspection introspection, QueryPhase queryPhase)
		: RdpShaderProgram(queryPhase)
	{
	...
	RdpShaderProgram::RdpShaderProgram(StringView vertexFile, StringView fragmentFile, Introspection introspection)
		: RdpShaderProgram(vertexFile, fragmentFile, introspection, QueryPhase::Immediate)
	{
	}

	RdpShaderProgram::RdpShaderProgram(StringView vertexFile, StringView fragmentFile)
		: RdpShaderProgram(vertexFile, fragmentFile, Introspection::Enabled, QueryPhase::Immediate)
	{
	}
	...
	bool RdpShaderProgram::IsLinked() const
	{
		return (_status == Status::Linked || _status == Status::LinkedWithDeferredQueries || _status == Status::LinkedWithIntrospection);
	}

	bool RdpShaderProgram::AttachShaderFromFile(ShaderStage stage, StringView filename)
	{
	...
	bool RdpShaderProgram::AttachShaderFromString(ShaderStage stage, StringView string)
	{
		static_cast<void>(stage);
	...
	bool RdpShaderProgram::AttachShaderFromStrings(ShaderStage stage, ArrayView<const StringView> strings)
	{
		static_cast<void>(stage);
	...
	bool RdpShaderProgram::AttachShaderFromStringsAndFile(ShaderStage stage, ArrayView<const StringView> strings, StringView filename)
	{
		static_cast<void>(stage);
		static_cast<void>(strings);
		static_cast<void>(filename);
	...
	bool RdpShaderProgram::Link(Introspection introspection)
	{
		return FinalizeAfterLinking(introspection);
	}

	bool RdpShaderProgram::FinalizeAfterLinking(Introspection introspection)
	{
	...
	void RdpShaderProgram::Use()
	{
		RdpDevice::BindProgram(this);
	}

	void RdpShaderProgram::PerformIntrospection()
	{
	...
	void RdpShaderProgram::ImportReflection()
	{
		...
		for (std::size_t i = 0; i < reflection.BlockCount; i++) {
			...
			RdpUniformBlock& block = _uniformBlocks.back();
	...
	bool RdpShaderProgram::HasAttribute(const char* name) const
	{
	...
	RdpVertexFormat::Attribute* RdpShaderProgram::GetAttribute(const char* name)
	{
		for (const RdpAttribute& a : _attributes) {
	...
	void RdpShaderProgram::DefineVertexFormat(const RdpBuffer* vbo, const RdpBuffer* ibo, std::uint32_t vboOffset)
	{
		...
		if (vbo != nullptr) {
			for (const RdpAttribute& a : _attributes) {
				if (a.GetLocation() >= 0) {
					RdpVertexFormat::Attribute& attr = _vertexFormat[std::uint32_t(a.GetLocation())];
					attr.setVbo(vbo);
	...
	void RdpShaderProgram::SetObjectLabel(StringView label)
	{
		// The label is kept for log messages only - effect identity comes from SetProgramIdentity()
		_label = label;
	}

	void RdpShaderProgram::SetProgramIdentity(const char* programName, const char* variantName)
	{
	...
	const RdpUniformBlock* RdpShaderProgram::FindBlock(const char* name) const
	{
		for (const RdpUniformBlock& block : _uniformBlocks) {
	...
	void RdpShaderProgram::SetResolvedUniform(const char* name, const std::uint8_t* data)
	{
	...
	const std::uint8_t* RdpShaderProgram::ResolveUniform(const char* name) const
	{
