...
#include "RdpShaderTypes.h"
#include "RdpVertexFormat.h"
#include "../RhiTypes.h"
...
namespace nCine::RHI::RDP
{
	...
	/**
		@brief Shader program of the RDP backend (aliased as `RHI::ShaderProgram`)
	...
	*/
	...
	{
		friend class RdpShaderUniforms;
		friend class RdpShaderUniformBlocks;
	...
	public:
		...
		RdpShaderProgram();
		explicit RdpShaderProgram(QueryPhase queryPhase);
		RdpShaderProgram(StringView vertexFile, StringView fragmentFile, Introspection introspection, QueryPhase queryPhase);
		RdpShaderProgram(StringView vertexFile, StringView fragmentFile, Introspection introspection);
		RdpShaderProgram(StringView vertexFile, StringView fragmentFile);
		~RdpShaderProgram();

		RdpShaderProgram(const RdpShaderProgram&) = delete;
		RdpShaderProgram& operator=(const RdpShaderProgram&) = delete;
		...
		*/
		...
		RdpVertexFormat::Attribute* GetAttribute(const char* name);

		inline void DefineVertexFormat(const RdpBuffer* vbo) {
			DefineVertexFormat(vbo, nullptr, 0);
		}
		inline void DefineVertexFormat(const RdpBuffer* vbo, const RdpBuffer* ibo) {
			DefineVertexFormat(vbo, ibo, 0);
		}
		void DefineVertexFormat(const RdpBuffer* vbo, const RdpBuffer* ibo, std::uint32_t vboOffset);
		/** @brief Returns the vertex buffer last bound by @ref DefineVertexFormat(), or `nullptr` */
		inline const RdpBuffer* GetBoundVbo() const {
			return _boundVbo;
		}
		...
		// -- RDP backend extensions (used by the device and the effects) --
		...
		*/
		struct DispatchFacts
		{
			...
			const RdpUniformBlock* InstanceBlock = nullptr;
		...
		const RdpUniformBlock* FindBlock(const char* name) const;
	...
	private:
		...
		std::vector<RdpUniform> _uniforms;
		std::vector<RdpUniformBlock> _uniformBlocks;
		std::vector<RdpAttribute> _attributes;
