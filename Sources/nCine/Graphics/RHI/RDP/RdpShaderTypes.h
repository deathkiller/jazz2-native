...
namespace nCine::RHI::RDP
{
	...
	/**
		...
		Holds everything the RDP backend needs about one uniform imported from the offline
		...
		value storage — a @ref RdpUniformCache holds the value.
	*/
	class RdpUniform
	{
		friend class RdpShaderProgram;
		friend class RdpUniformBlock;
	...
	public:
		...
		RdpUniform(RdpShaderProgram* owner, const char* name, ShaderCompiler::UniformType type, std::int32_t arraySize, std::int32_t location)
		...
		inline RdpShaderProgram* GetOwner() const {
			return _owner;
		}
	...
	/**
		...
		metadata of its member uniforms (each with a std140 offset). The RDP backend does not pad the
		size to any device alignment, so @ref GetAlignAmount() is always 0.
	*/
	class RdpUniformBlock
	{
		friend class RdpShaderProgram;
		friend class RdpUniformBlockCache;
	...
	public:
		...
		RdpUniformBlock(std::uint32_t index, const char* name, std::int32_t dataSize)
		...
		RdpUniform* GetUniform(const char* name) {
			for (RdpUniform& u : _members) {
	...
	private:
		...
		std::vector<RdpUniform> _members;
	...
	*/
	...
	public:
		...
		RdpAttribute(const char* name, ShaderCompiler::UniformType type, std::int32_t location)
