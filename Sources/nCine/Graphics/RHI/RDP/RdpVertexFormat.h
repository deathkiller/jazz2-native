...
namespace nCine::RHI::RDP
{
	...
	/**
		@brief Vertex layout description of the RDP backend (aliased as `RHI::VertexFormat`)

		Records the set of vertex attributes (component count, type, stride, offset and source buffer) the
		way the OpenGL backend does, so the pipeline's attribute setup code compiles unchanged. The RDP draw
		dispatch synthesizes sprite corners from the per-draw uniform block and submits finished screen-space
		primitives through rdpq with its own layout, so the recorded layout is currently informational -
	...
	*/
	...
	public:
		...
		public:
			...
			// needing the complete `RdpBuffer` type in this header
			...
			inline const RdpBuffer* GetVbo() const {
				return _vbo;
			}
			...
			inline void setVbo(const RdpBuffer* vbo) {
				_vbo = vbo;
			}
		...
		inline const RdpBuffer* GetIbo() const {
			return _ibo;
		}
		inline void SetIbo(const RdpBuffer* ibo) {
			_ibo = ibo;
		}
		/** @brief Applies the vertex format (no-op for the RDP backend) */
		void Define() {}
		...
		bool operator==(const RdpVertexFormat& other) const {
	...
	private:
		...
		const RdpBuffer* _ibo;
	};

	/**
		@brief Vertex array object stub of the RDP backend (aliased as `RHI::VertexArray`)

		The RDP has no vertex-array state object to capture, so this only satisfies the contract alias with
		inert bind/label operations.
	*/
	class RdpVertexArray
	{
	public:
		RdpVertexArray() = default;
		~RdpVertexArray() = default;

		RdpVertexArray(const RdpVertexArray&) = delete;
		RdpVertexArray& operator=(const RdpVertexArray&) = delete;
