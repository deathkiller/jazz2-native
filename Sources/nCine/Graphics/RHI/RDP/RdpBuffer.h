...
{
	/**
		@brief Host-memory buffer object of the RDP backend (aliased as `RHI::Buffer`)

		The RDP backend consumes vertices and uniform blocks from host memory (the CPU transforms every
		primitive itself and enqueues finished rdpq commands, so there is no persistent device-side vertex
		store to upload into), which makes this class a plain resizable byte store exactly like the software
		backend's: @ref
	...
	*/
	...
	{
	public:
		explicit RdpBuffer(BufferTarget target);
		~RdpBuffer() = default;

		RdpBuffer(const RdpBuffer&) = delete;
		RdpBuffer& operator=(const RdpBuffer&) = delete;
		...
		/** @brief Sets a debug label for the buffer (ignored by the RDP backend) */
		...
		/** @brief Records the buffer handle bound for a target (inert for the RDP backend) */
		inline static void SetBoundHandle(std::uint32_t target, std::uint32_t glHandle) {
			static_cast<void>(target);
			static_cast<void>(glHandle);
		}
		/** @brief Marks a buffer handle as bound for a target (inert for the RDP backend) */
