...
namespace nCine::RHI::RDP
{
	/**
		...
		block, committing is a no-op — the whole block is published by @ref RdpShaderUniformBlocks.
	*/
	...
	public:
		...
		explicit RdpUniformCache(const RdpUniform* uniform)
			: _uniform(uniform), _dataPointer(nullptr), _isDirty(false) {}

		inline const RdpUniform* GetUniform() const {
			return _uniform;
		}
	...
	/**
		...
		RdpUniformCache for each member, each pointing at its member offset within that buffer. The block is
		uploaded as one contiguous range by @ref RdpShaderUniformBlocks.
	*/
	class RdpUniformBlockCache
	{
	public:
		RdpUniformBlockCache()
			: _uniformBlock(nullptr), _dataPointer(nullptr), _usedSize(0) {}
		explicit RdpUniformBlockCache(RdpUniformBlock* uniformBlock);

		inline const RdpUniformBlock* uniformBlock() const {
		...
		RdpUniformCache* GetUniform(StringView name);
		void SetBlockBinding(std::int32_t blockBinding);

	private:
		RdpUniformBlock* _uniformBlock;
		std::uint8_t* _dataPointer;
		...
		{
			String Name;
			RdpUniformCache Cache;
		};
		std::vector<NamedCache> _uniformCaches;
		// Name fingerprints of _uniformCaches, in the same order and kept in their own contiguous array
		// so a lookup touches only these few bytes (see HashUniformName)
