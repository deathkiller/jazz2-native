...
namespace nCine::RHI::PVR
{
	...
	/**
		...
		Owns a @ref PvrUniformCache for every active uniform of a program that is not part of a block,
	...
	*/
	...
	{
	public:
		// The PVR backend stores the caches in a flat container whose value type is the cache
		// itself (the name lives inside each cache's uniform), so range-iteration yields `PvrUniformCache&`
		...
		using UniformHashMapType = std::vector<PvrUniformCache>;

		PvrShaderUniforms();
		explicit PvrShaderUniforms(PvrShaderProgram* shaderProgram);
		PvrShaderUniforms(PvrShaderProgram* shaderProgram, const char* includeOnly, const char* exclude);

		inline void SetProgram(PvrShaderProgram* shaderProgram) {
			SetProgram(shaderProgram, nullptr, nullptr);
		}
		void SetProgram(PvrShaderProgram* shaderProgram, const char* includeOnly, const char* exclude);
		...
		PvrUniformCache* GetUniform(const char* name);
		/** @brief Returns the container of all managed uniform caches (range-iterates as `PvrUniformCache&`) */
	...
	private:
		PvrShaderProgram* shaderProgram_;
		bool maybeDirty_;
	...
	*/
	...
	public:
		...
		// block), so range-iteration yields `PvrUniformBlockCache&` directly, matching the OpenGL backend.
		using UniformHashMapType = std::vector<PvrUniformBlockCache>;
		...
		explicit PvrShaderUniformBlocks(PvrShaderProgram* shaderProgram);
		PvrShaderUniformBlocks(PvrShaderProgram* shaderProgram, const char* includeOnly, const char* exclude);
		...
		PvrUniformBlockCache* GetUniformBlock(const char* name);
		/** @brief Returns the container of all managed block caches (range-iterates as `PvrUniformBlockCache&`) */
