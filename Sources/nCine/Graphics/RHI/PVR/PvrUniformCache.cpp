...
namespace nCine::RHI::PVR
{
	...
	bool PvrUniformCache::SetFloatValue(float v0)
	{
	...
	bool PvrUniformCache::SetFloatValue(float v0, float v1)
	{
	...
	bool PvrUniformCache::SetFloatValue(float v0, float v1, float v2)
	{
	...
	bool PvrUniformCache::SetFloatValue(float v0, float v1, float v2, float v3)
	{
	...
	bool PvrUniformCache::SetIntVector(const std::int32_t* vec)
	{
	...
	bool PvrUniformCache::SetIntValue(std::int32_t v0)
	{
	...
	bool PvrUniformCache::SetIntValue(std::int32_t v0, std::int32_t v1)
	{
	...
	bool PvrUniformCache::SetIntValue(std::int32_t v0, std::int32_t v1, std::int32_t v2)
	{
	...
	bool PvrUniformCache::SetIntValue(std::int32_t v0, std::int32_t v1, std::int32_t v2, std::int32_t v3)
	{
	...
	bool PvrUniformCache::CommitValue()
	{
	...
	bool PvrUniformCache::CheckFloat() const
	{
		return (_uniform != nullptr && _uniform->IsFloat());
	}

	bool PvrUniformCache::CheckInt() const
	{
		return (_uniform != nullptr && !_uniform->IsFloat());
	}

	bool PvrUniformCache::CheckComponents(std::uint32_t requiredComponents) const
	{
	...
	PvrUniformBlockCache::PvrUniformBlockCache(PvrUniformBlock* uniformBlock)
		: _uniformBlock(uniformBlock), _dataPointer(nullptr), _usedSize(0)
	{
		...
		for (PvrUniform& member : uniformBlock->_members) {
			_uniformCaches.push_back({member.GetName(), PvrUniformCache(&member)});
			_uniformNameHashes.push_back(HashUniformName(member.GetName()));
		}
	}

	std::uint32_t PvrUniformBlockCache::GetIndex() const
	{
		return (_uniformBlock != nullptr ? _uniformBlock->GetIndex() : 0);
	}

	std::int32_t PvrUniformBlockCache::GetBindingIndex() const
	{
		return (_uniformBlock != nullptr ? _uniformBlock->GetBindingIndex() : 0);
	}

	std::int32_t PvrUniformBlockCache::GetSize() const
	{
		return (_uniformBlock != nullptr ? _uniformBlock->GetSize() : 0);
	}

	std::uint8_t PvrUniformBlockCache::GetAlignAmount() const
	{
		return (_uniformBlock != nullptr ? _uniformBlock->GetAlignAmount() : 0);
	}

	void PvrUniformBlockCache::SetDataPointer(std::uint8_t* dataPointer)
	{
	...
	void PvrUniformBlockCache::SetUsedSize(std::int32_t usedSize)
	{
		if (usedSize >= 0) {
			_usedSize = usedSize;
		}
	...
	bool PvrUniformBlockCache::CopyData(std::uint32_t destIndex, const std::uint8_t* src, std::uint32_t numBytes)
	{
	...
	PvrUniformCache* PvrUniformBlockCache::GetUniform(StringView name)
	{
	...
	void PvrUniformBlockCache::SetBlockBinding(std::int32_t blockBinding)
	{
