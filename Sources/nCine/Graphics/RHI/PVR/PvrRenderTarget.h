...
namespace nCine::RHI::PVR
{
	...
	/**
		@brief Renderbuffer stub of the PVR backend (aliased as `RHI::Renderbuffer`)

		The PVR backend renders 2D only, so depth/stencil renderbuffers carry no storage; the class
	...
	*/
	...
	/**
		@brief Framebuffer stub of the PVR backend (aliased as `RHI::Framebuffer`)

		Provided only for the contract alias (the default-framebuffer rebinding some window backends use).
		The PVR backend routes off-screen rendering through @ref PvrRenderTarget instead.
	*/
	class PvrFramebuffer
	{
	public:
		PvrFramebuffer() = default;
		inline std::uint32_t GetGLHandle() const {
			return 0;
		}
		bool Bind() const {
	...
	/**
		@brief Off-screen render target of the PVR backend (aliased as `RHI::RenderTarget`)
	...
	*/
	...
	public:
		...
		PvrRenderTarget(const PvrRenderTarget&) = delete;
		PvrRenderTarget& operator=(const PvrRenderTarget&) = delete;

		/** @brief Attaches a texture as the color attachment with the given index */
		void AttachColorTexture(PvrTexture& texture, std::uint32_t index);
		/** @brief Detaches any texture from the color attachment with the given index */
		void DetachColorTexture(std::uint32_t index);

		/** @brief Records a depth/stencil buffer (no storage — the PVR backend is 2D) */
