...
{
	std::uint32_t PvrShaderProgram::_nextHandle = 1;
	...
	void PvrShaderProgram::PerformIntrospection()
	{
		if (_introspection != Introspection::Disabled && _status != Status::LinkedWithIntrospection) {
			...
			_generatedEffect = (!_programName.empty()
				? PvrDevice::FindGeneratedEffect(_programName.data(), _variantName.data()) : nullptr);
	...
	void PvrShaderProgram::ImportReflection()
	{
		...
		for (std::size_t i = 0; i < reflection.BlockCount; i++) {
			...
			PvrUniformBlock& block = _uniformBlocks.back();
	...
	{
		for (const PvrAttribute& a : _attributes) {
	...
	void PvrShaderProgram::DefineVertexFormat(const PvrBuffer* vbo, const PvrBuffer* ibo, std::uint32_t vboOffset)
	{
		...
		if (vbo != nullptr) {
			for (const PvrAttribute& a : _attributes) {
				if (a.GetLocation() >= 0) {
					PvrVertexFormat::Attribute& attr = _vertexFormat[std::uint32_t(a.GetLocation())];
					attr.setVbo(vbo);
	...
	{
		for (const PvrUniformBlock& block : _uniformBlocks) {
