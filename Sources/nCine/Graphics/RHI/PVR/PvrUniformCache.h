...
namespace nCine::RHI::PVR
{
	/**
		...
		block, committing is a no-op — the whole block is published by @ref PvrShaderUniformBlocks.
	*/
	...
	public:
		...
		explicit PvrUniformCache(const PvrUniform* uniform)
			: _uniform(uniform), _dataPointer(nullptr), _isDirty(false) {}

		inline const PvrUniform* GetUniform() const {
			return _uniform;
		}
	...
	/**
		...
		PvrUniformCache for each member, each pointing at its member offset within that buffer. The block is
		uploaded as one contiguous range by @ref PvrShaderUniformBlocks.
	*/
	class PvrUniformBlockCache
	{
	public:
		PvrUniformBlockCache()
			: _uniformBlock(nullptr), _dataPointer(nullptr), _usedSize(0) {}
		explicit PvrUniformBlockCache(PvrUniformBlock* uniformBlock);

		inline const PvrUniformBlock* uniformBlock() const {
		...
		PvrUniformCache* GetUniform(StringView name);
		void SetBlockBinding(std::int32_t blockBinding);

	private:
		PvrUniformBlock* _uniformBlock;
		std::uint8_t* _dataPointer;
