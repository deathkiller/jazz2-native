...
namespace nCine::RHI::PVR
{
	...
	/**
		...
		Holds everything the PVR backend needs about one uniform imported from the offline
		...
		value storage — a @ref PvrUniformCache holds the value.
	*/
	class PvrUniform
	{
		friend class PvrShaderProgram;
		friend class PvrUniformBlock;
	...
	public:
		...
		PvrUniform(PvrShaderProgram* owner, const char* name, ShaderCompiler::UniformType type, std::int32_t arraySize, std::int32_t location)
		...
		inline PvrShaderProgram* GetOwner() const {
			return owner_;
		}
	...
	/**
		...
		metadata of its member uniforms (each with a std140 offset). The PVR backend does not pad the
		size to any device alignment, so @ref GetAlignAmount() is always 0.
	*/
	class PvrUniformBlock
	{
		friend class PvrShaderProgram;
		friend class PvrUniformBlockCache;
	...
	public:
		...
		PvrUniformBlock(std::uint32_t index, const char* name, std::int32_t dataSize)
		...
		PvrUniform* GetUniform(const char* name) {
			for (PvrUniform& u : members_) {
	...
	private:
		...
		std::vector<PvrUniform> members_;
