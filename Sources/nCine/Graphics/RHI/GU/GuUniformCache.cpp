...
namespace nCine::RHI::GU
{
	...
	bool GuUniformCache::SetFloatValue(float v0)
	{
	...
	bool GuUniformCache::SetFloatValue(float v0, float v1)
	{
	...
	bool GuUniformCache::SetFloatValue(float v0, float v1, float v2)
	{
	...
	bool GuUniformCache::SetFloatValue(float v0, float v1, float v2, float v3)
	{
	...
	bool GuUniformCache::SetIntVector(const std::int32_t* vec)
	{
	...
	bool GuUniformCache::SetIntValue(std::int32_t v0)
	{
	...
	bool GuUniformCache::SetIntValue(std::int32_t v0, std::int32_t v1)
	{
	...
	bool GuUniformCache::SetIntValue(std::int32_t v0, std::int32_t v1, std::int32_t v2)
	{
	...
	bool GuUniformCache::SetIntValue(std::int32_t v0, std::int32_t v1, std::int32_t v2, std::int32_t v3)
	{
	...
	bool GuUniformCache::CommitValue()
	{
	...
	bool GuUniformCache::CheckFloat() const
	{
		return (uniform_ != nullptr && uniform_->IsFloat());
	}

	bool GuUniformCache::CheckInt() const
	{
		return (uniform_ != nullptr && !uniform_->IsFloat());
	}

	bool GuUniformCache::CheckComponents(std::uint32_t requiredComponents) const
	{
	...
	GuUniformBlockCache::GuUniformBlockCache(GuUniformBlock* uniformBlock)
		: uniformBlock_(uniformBlock), dataPointer_(nullptr), usedSize_(0)
	{
		...
		for (GuUniform& member : uniformBlock->members_) {
			uniformCaches_.push_back({member.GetName(), GuUniformCache(&member)});
		}
	}

	std::uint32_t GuUniformBlockCache::GetIndex() const
	{
		return (uniformBlock_ != nullptr ? uniformBlock_->GetIndex() : 0);
	}

	std::int32_t GuUniformBlockCache::GetBindingIndex() const
	{
		return (uniformBlock_ != nullptr ? uniformBlock_->GetBindingIndex() : 0);
	}

	std::int32_t GuUniformBlockCache::GetSize() const
	{
		return (uniformBlock_ != nullptr ? uniformBlock_->GetSize() : 0);
	}

	std::uint8_t GuUniformBlockCache::GetAlignAmount() const
	{
		return (uniformBlock_ != nullptr ? uniformBlock_->GetAlignAmount() : 0);
	}

	void GuUniformBlockCache::SetDataPointer(std::uint8_t* dataPointer)
	{
	...
	void GuUniformBlockCache::SetUsedSize(std::int32_t usedSize)
	{
		if (usedSize >= 0) {
			usedSize_ = usedSize;
		}
	...
	bool GuUniformBlockCache::CopyData(std::uint32_t destIndex, const std::uint8_t* src, std::uint32_t numBytes)
	{
	...
	GuUniformCache* GuUniformBlockCache::GetUniform(StringView name)
	{
		for (NamedCache& named : uniformCaches_) {
			if (name == named.Name) {
				return &named.Cache;
	...
	void GuUniformBlockCache::SetBlockBinding(std::int32_t blockBinding)
	{
