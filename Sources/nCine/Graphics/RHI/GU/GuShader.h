...
{
	/**
		@brief Shader-object stub of the GU backend
	...
	*/
	...
	{
	public:
		/** @brief Compilation status of the shader (always @ref Status::Compiled for the GU backend) */
		enum class Status
		{
			NotCompiled,
			CompilationFailed,
		...
		/** @brief When the compilation status is checked (irrelevant for the GU backend) */
		...
		explicit GuShader(std::uint32_t type)
