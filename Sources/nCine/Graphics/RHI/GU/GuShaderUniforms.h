...
namespace nCine::RHI::GU
{
	...
	/**
		...
		Owns a @ref GuUniformCache for every active uniform of a program that is not part of a block,
	...
	*/
	...
	{
	public:
		// The GU backend stores the caches in a flat container whose value type is the cache
		// itself (the name lives inside each cache's uniform), so range-iteration yields `GuUniformCache&`
		...
		using UniformHashMapType = std::vector<GuUniformCache>;

		GuShaderUniforms();
		explicit GuShaderUniforms(GuShaderProgram* shaderProgram);
		GuShaderUniforms(GuShaderProgram* shaderProgram, const char* includeOnly, const char* exclude);

		inline void SetProgram(GuShaderProgram* shaderProgram) {
		...
		GuUniformCache* GetUniform(const char* name);
		/** @brief Returns the container of all managed uniform caches (range-iterates as `GuUniformCache&`) */
	...
	private:
		GuShaderProgram* _shaderProgram;
		bool _maybeDirty;
	...
	/**
		...
		Owns a @ref GuUniformBlockCache per active block, distributes a shared host data buffer across
	...
	*/
	...
	public:
		...
		// block), so range-iteration yields `GuUniformBlockCache&` directly, matching the OpenGL backend.
		...
		explicit GuShaderUniformBlocks(GuShaderProgram* shaderProgram);
		...
		GuUniformBlockCache* GetUniformBlock(const char* name);
		/** @brief Returns the container of all managed block caches (range-iterates as `GuUniformBlockCache&`) */
