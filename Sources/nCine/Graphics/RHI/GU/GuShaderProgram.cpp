...
{
	std::uint32_t GuShaderProgram::_nextHandle = 1;

	GuShaderProgram::GuShaderProgram()
		: GuShaderProgram(QueryPhase::Immediate)
	{
	}

	GuShaderProgram::GuShaderProgram(QueryPhase queryPhase)
	...
	GuShaderProgram::GuShaderProgram(StringView vertexFile, StringView fragmentFile, Introspection introspection, QueryPhase queryPhase)
		: GuShaderProgram(queryPhase)
	{
	...
	GuShaderProgram::GuShaderProgram(StringView vertexFile, StringView fragmentFile, Introspection introspection)
		: GuShaderProgram(vertexFile, fragmentFile, introspection, QueryPhase::Immediate)
	{
	}

	GuShaderProgram::GuShaderProgram(StringView vertexFile, StringView fragmentFile)
		: GuShaderProgram(vertexFile, fragmentFile, Introspection::Enabled, QueryPhase::Immediate)
	{
	}
	...
	bool GuShaderProgram::IsLinked() const
	{
		return (_status == Status::Linked || _status == Status::LinkedWithDeferredQueries || _status == Status::LinkedWithIntrospection);
	}

	bool GuShaderProgram::AttachShaderFromFile(ShaderStage stage, StringView filename)
	{
	...
	bool GuShaderProgram::AttachShaderFromString(ShaderStage stage, StringView string)
	{
		static_cast<void>(stage);
	...
	bool GuShaderProgram::AttachShaderFromStrings(ShaderStage stage, ArrayView<const StringView> strings)
	{
		static_cast<void>(stage);
	...
	bool GuShaderProgram::AttachShaderFromStringsAndFile(ShaderStage stage, ArrayView<const StringView> strings, StringView filename)
	{
		static_cast<void>(stage);
		static_cast<void>(strings);
		static_cast<void>(filename);
	...
	bool GuShaderProgram::Link(Introspection introspection)
	{
		return FinalizeAfterLinking(introspection);
	}

	bool GuShaderProgram::FinalizeAfterLinking(Introspection introspection)
	{
	...
	void GuShaderProgram::Use()
	{
		GuDevice::BindProgram(this);
	}

	void GuShaderProgram::PerformIntrospection()
	{
		if (_introspection != Introspection::Disabled && _status != Status::LinkedWithIntrospection) {
			...
			_generatedEffect = (!_programName.empty()
				? GuDevice::FindGeneratedEffect(_programName.data(), _variantName.data()) : nullptr);
	...
	void GuShaderProgram::ImportReflection()
	{
		...
		for (std::size_t i = 0; i < reflection.BlockCount; i++) {
			...
			GuUniformBlock& block = _uniformBlocks.back();
	...
	bool GuShaderProgram::HasAttribute(const char* name) const
	{
		for (const GuAttribute& a : _attributes) {
	...
	GuVertexFormat::Attribute* GuShaderProgram::GetAttribute(const char* name)
	{
		for (const GuAttribute& a : _attributes) {
	...
	void GuShaderProgram::DefineVertexFormat(const GuBuffer* vbo, const GuBuffer* ibo, std::uint32_t vboOffset)
	{
		_boundVbo = vbo;
		...
		if (vbo != nullptr) {
			for (const GuAttribute& a : _attributes) {
				if (a.GetLocation() >= 0) {
					GuVertexFormat::Attribute& attr = _vertexFormat[std::uint32_t(a.GetLocation())];
					attr.setVbo(vbo);
	...
	void GuShaderProgram::SetProgramIdentity(const char* programName, const char* variantName)
	{
	...
	const GuUniformBlock* GuShaderProgram::FindBlock(const char* name) const
	{
		for (const GuUniformBlock& block : _uniformBlocks) {
	...
	void GuShaderProgram::SetResolvedUniform(const char* name, const std::uint8_t* data)
	{
	...
	const std::uint8_t* GuShaderProgram::ResolveUniform(const char* name) const
	{
