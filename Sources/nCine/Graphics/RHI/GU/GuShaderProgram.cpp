...
{
	std::uint32_t GuShaderProgram::nextHandle_ = 1;
	...
	void GuShaderProgram::PerformIntrospection()
	{
		if (introspection_ != Introspection::Disabled && status_ != Status::LinkedWithIntrospection) {
			...
			generatedEffect_ = (!programName_.empty()
				? GuDevice::FindGeneratedEffect(programName_.data(), variantName_.data()) : nullptr);
	...
	void GuShaderProgram::ImportReflection()
	{
		...
		for (std::size_t i = 0; i < reflection.BlockCount; i++) {
			...
			GuUniformBlock& block = uniformBlocks_.back();
	...
	{
		for (const GuAttribute& a : attributes_) {
	...
	void GuShaderProgram::DefineVertexFormat(const GuBuffer* vbo, const GuBuffer* ibo, std::uint32_t vboOffset)
	{
		...
		if (vbo != nullptr) {
			for (const GuAttribute& a : attributes_) {
				if (a.GetLocation() >= 0) {
					GuVertexFormat::Attribute& attr = vertexFormat_[std::uint32_t(a.GetLocation())];
					attr.setVbo(vbo);
	...
	{
		for (const GuUniformBlock& block : uniformBlocks_) {
