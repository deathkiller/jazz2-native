...
namespace nCine::RHI::GU
{
	/**
		...
		block, committing is a no-op — the whole block is published by @ref GuShaderUniformBlocks.
	*/
	...
	public:
		...
		explicit GuUniformCache(const GuUniform* uniform)
			: _uniform(uniform), _dataPointer(nullptr), _isDirty(false) {}

		inline const GuUniform* GetUniform() const {
			return _uniform;
		}
	...
	/**
		...
		GuUniformCache for each member, each pointing at its member offset within that buffer. The block is
		uploaded as one contiguous range by @ref GuShaderUniformBlocks.
	*/
	...
	public:
		...
		explicit GuUniformBlockCache(GuUniformBlock* uniformBlock);

		inline const GuUniformBlock* uniformBlock() const {
		...
		GuUniformCache* GetUniform(StringView name);
		void SetBlockBinding(std::int32_t blockBinding);

	private:
		GuUniformBlock* _uniformBlock;
		std::uint8_t* _dataPointer;
