...
namespace nCine::RHI::GU
{
	...
	/**
		@brief Renderbuffer stub of the GU backend (aliased as `RHI::Renderbuffer`)

		The GU backend renders 2D only, so depth/stencil renderbuffers carry no storage; the class
		just records the format and size to satisfy the contract alias.
	*/
	class GuRenderbuffer
	{
	public:
		GuRenderbuffer() = default;
	...
	/**
		@brief Framebuffer stub of the GU backend (aliased as `RHI::Framebuffer`)

		Provided only for the contract alias (the default-framebuffer rebinding some window backends use).
		The GU backend routes off-screen rendering through @ref GuRenderTarget instead.
	*/
	class GuFramebuffer
	{
	public:
		GuFramebuffer() = default;
		inline std::uint32_t GetGLHandle() const {
			return 0;
		}
		bool Bind() const {
	...
	/**
		@brief Off-screen render target of the GU backend (aliased as `RHI::RenderTarget`)
	...
	*/
	...
	public:
		...
		GuRenderTarget(const GuRenderTarget&) = delete;
		GuRenderTarget& operator=(const GuRenderTarget&) = delete;

		/** @brief Attaches a texture as the color attachment with the given index */
		void AttachColorTexture(GuTexture& texture, std::uint32_t index);
		/** @brief Detaches any texture from the color attachment with the given index */
		void DetachColorTexture(std::uint32_t index);

		/** @brief Records a depth/stencil buffer (no storage — the GU backend is 2D) */
		...
		inline GuTexture* GetColorTexture(std::uint32_t index) const {
			return (index < MaxColorAttachments ? colorTextures_[index] : nullptr);
		}

	private:
		GuTexture* colorTextures_[MaxColorAttachments];
