...
{
	/**
		@brief Host-memory buffer object of the GU backend (aliased as `RHI::Buffer`)

		The GU backend consumes vertices and uniform blocks from host memory (GE draws are fed through a
	...
	*/
	...
	{
	public:
		explicit GuBuffer(BufferTarget target);
		~GuBuffer() = default;

		GuBuffer(const GuBuffer&) = delete;
		GuBuffer& operator=(const GuBuffer&) = delete;
		...
		/** @brief Sets a debug label for the buffer (ignored by the GU backend) */
		...
		/** @brief Records the buffer handle bound for a target (inert for the GU backend) */
		inline static void SetBoundHandle(std::uint32_t target, std::uint32_t glHandle) {
			static_cast<void>(target);
			static_cast<void>(glHandle);
		}
		/** @brief Marks a buffer handle as bound for a target (inert for the GU backend) */
