...
#include "GuShaderTypes.h"
#include "GuVertexFormat.h"
#include "../RhiTypes.h"
...
namespace nCine::RHI::GU
{
	...
	/**
		@brief Shader program of the GU backend (aliased as `RHI::ShaderProgram`)
	...
	*/
	...
	{
		friend class GuShaderUniforms;
		friend class GuShaderUniformBlocks;
	...
	public:
		...
		GuShaderProgram();
		explicit GuShaderProgram(QueryPhase queryPhase);
		GuShaderProgram(StringView vertexFile, StringView fragmentFile, Introspection introspection, QueryPhase queryPhase);
		GuShaderProgram(StringView vertexFile, StringView fragmentFile, Introspection introspection);
		GuShaderProgram(StringView vertexFile, StringView fragmentFile);
		~GuShaderProgram();

		GuShaderProgram(const GuShaderProgram&) = delete;
		GuShaderProgram& operator=(const GuShaderProgram&) = delete;
		...
		*/
		...
		GuVertexFormat::Attribute* GetAttribute(const char* name);

		inline void DefineVertexFormat(const GuBuffer* vbo) {
			DefineVertexFormat(vbo, nullptr, 0);
		}
		inline void DefineVertexFormat(const GuBuffer* vbo, const GuBuffer* ibo) {
			DefineVertexFormat(vbo, ibo, 0);
		}
		void DefineVertexFormat(const GuBuffer* vbo, const GuBuffer* ibo, std::uint32_t vboOffset);
		/** @brief Returns the vertex buffer last bound by @ref DefineVertexFormat(), or `nullptr` */
		inline const GuBuffer* GetBoundVbo() const {
			return _boundVbo;
		}
		...
		inline const GuBuffer* GetBoundIbo() const {
			return _boundIbo;
		}
		...
		// -- GU backend extensions (used by the device and the effects) --
		...
		*/
		...
		const GuUniformBlock* FindBlock(const char* name) const;
	...
	private:
		...
		std::vector<GuUniform> _uniforms;
		std::vector<GuUniformBlock> _uniformBlocks;
		std::vector<GuAttribute> _attributes;
		...
		GuVertexFormat _vertexFormat;
		const GuBuffer* _boundVbo;
		std::uint32_t _boundVboOffset;
		const GuBuffer* _boundIbo;
