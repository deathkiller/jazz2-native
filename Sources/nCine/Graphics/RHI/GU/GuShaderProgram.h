...
namespace nCine::RHI::GU
{
	...
	*/
	...
	private:
		...
		std::vector<GuUniform> uniforms_;
		std::vector<GuUniformBlock> uniformBlocks_;
		std::vector<GuAttribute> attributes_;
		...
		GuVertexFormat vertexFormat_;
		const GuBuffer* boundVbo_;
		std::uint32_t boundVboOffset_;
		const GuBuffer* boundIbo_;
