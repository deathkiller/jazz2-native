...
namespace nCine::RhiSoftware
{
	...
	namespace TileInternal
	{
		...
		// (TileSize stride), not the full framebuffer.
		// =====================================================================
		                                   ...
		                                   std::int32_t tileW, std::int32_t tileH)
		{
			...
			// PARANOID GUARD: a zero-dimension texture causes % 0 (division by zero) in WrapTexelFix /
			// NormalizeRepeatFix, which kills the worker thread on ARM.
			...
			static_cast<void>(texHFix); // Carried for parity with the immediate path; the Y walk uses tyFix directly
		                               ...
		                               std::int32_t tileW, std::int32_t tileH)
		{
			...
			// The per-edge dy step terms are carried for parity with the source but unused: this rasterizer
			// recomputes the edge functions from scratch each row instead of stepping them by dy.
			static_cast<void>(t1_w0_dy); static_cast<void>(t1_w1_dy); static_cast<void>(t1_w2_dy);
			static_cast<void>(t2_w0_dy); static_cast<void>(t2_w1_dy); static_cast<void>(t2_w2_dy);
			...
			// PARANOID GUARD: a zero-dimension texture causes % 0 in WrapTexelFix → thread kill
			if DEATH_UNLIKELY(texPixels != nullptr && (texW <= 0 || texH <= 0)) {
				return;
			}
		                         ...
		                         std::int32_t viewportW, std::int32_t viewportH)
		{
			...
			if DEATH_LIKELY(type == PrimitiveType::TriangleStrip && count == 4 && firstVertex == 0 &&
				...
				Vertex2D v0 = FetchVertex(ctx, 0, viewportX, viewportY, viewportW, viewportH);
				Vertex2D v1 = FetchVertex(ctx, 1, viewportX, viewportY, viewportW, viewportH);
				Vertex2D v2 = FetchVertex(ctx, 2, viewportX, viewportY, viewportW, viewportH);
				Vertex2D v3 = FetchVertex(ctx, 3, viewportX, viewportY, viewportW, viewportH);
