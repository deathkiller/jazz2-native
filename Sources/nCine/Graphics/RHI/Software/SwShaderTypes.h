...
namespace nCine::RhiSoftware
{
	...
	/**
		...
		value storage — a @ref SwUniformCache holds the value.
	*/
	class SwUniform
	{
		friend class SwShaderProgram;
		friend class SwUniformBlock;
	...
	public:
		...
		inline SwShaderProgram* GetOwner() const {
			return owner_;
		}
	...
	*/
	...
	{
		friend class SwShaderProgram;
		friend class SwUniformBlockCache;
	...
	public:
		...
		SwUniformBlock(std::uint32_t index, const char* name, std::int32_t dataSize)
		...
		SwUniform* GetUniform(const char* name) {
			for (SwUniform& u : members_) {
