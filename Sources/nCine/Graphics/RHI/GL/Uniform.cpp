...
namespace nCine::RHI
{
	...
	Uniform::Uniform(GLuint program, GLuint index)
		: Uniform()
	{
		GLsizei length;
	...
	GLenum Uniform::GetBasicType() const
	{
		switch (type_) {
			case GL_FLOAT:
			case GL_FLOAT_VEC2:
	...
	std::uint32_t Uniform::GetComponentCount() const
	{
		switch (type_) {
			case GL_FLOAT:
			case GL_INT:
	...
	bool Uniform::HasReservedPrefix() const
	{
