...
namespace nCine::RHI
{
	...
	ShaderUniforms::ShaderUniforms(ShaderProgram* shaderProgram)
		: ShaderUniforms(shaderProgram, nullptr, nullptr)
	{
		SetProgram(shaderProgram);
	}

	ShaderUniforms::ShaderUniforms(ShaderProgram* shaderProgram, const char* includeOnly, const char* exclude)
		: ShaderUniforms()
	{
		SetProgram(shaderProgram, includeOnly, exclude);
	}

	void ShaderUniforms::SetProgram(ShaderProgram* shaderProgram, const char* includeOnly, const char* exclude)
	{
	...
	void ShaderUniforms::SetUniformsDataPointer(GLubyte* dataPointer)
	{
	...
	void ShaderUniforms::SetDirty(bool isDirty)
	{
		if (shaderProgram_->GetStatus() != ShaderProgram::Status::LinkedWithIntrospection) {
			return;
		}
	...
	UniformCache* ShaderUniforms::GetUniform(const char* name)
	{
	...
	void ShaderUniforms::CommitUniforms()
	{
		if (shaderProgram_ != nullptr) {
			if (shaderProgram_->GetStatus() == ShaderProgram::Status::LinkedWithIntrospection) {
				shaderProgram_->Use();
	...
	void ShaderUniforms::ImportUniforms(const char* includeOnly, const char* exclude)
	{
		constexpr std::uint32_t MaxUniformName = 128;

		std::uint32_t importedCount = 0;
		for (const Uniform& uniform : shaderProgram_->uniforms_) {
