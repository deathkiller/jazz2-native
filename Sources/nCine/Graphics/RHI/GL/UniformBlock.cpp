...
namespace nCine::RHI
{
	...
	UniformBlock::UniformBlock(GLuint program, GLuint index, DiscoverUniforms discover)
		: UniformBlock()
	{
		GLint nameLength = 0;
...
#if !defined(DEATH_TARGET_EMSCRIPTEN)
				DEATH_ASSERT(uniformNameLengths[i] <= Uniform::MaxNameLength,
					("Uniform {} name length is {}, which is more than {}", i, uniformNameLengths[i], Uniform::MaxNameLength), );
#endif
	...
	UniformBlock::UniformBlock(GLuint program, GLuint index)
		: UniformBlock(program, index, DiscoverUniforms::ENABLED)
	{
	}

	void UniformBlock::SetBlockBinding(GLuint blockBinding)
	{
