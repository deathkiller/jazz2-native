...
namespace nCine::RHI
{
	...
	bool Debug::debugAvailable_ = false;
	GLuint Debug::debugGroupId_ = 0;
	std::int32_t Debug::maxLabelLength_ = 0;

	void Debug::Init(const IGfxCapabilities& gfxCaps)
	{
#if defined(GL_DEBUG_SUPPORTED)
	...
	void Debug::PushGroup(StringView message)
	{
#if defined(GL_DEBUG_SUPPORTED)
		if (debugAvailable_) {
	...
	void Debug::MessageInsert(StringView message)
	{
#if defined(GL_DEBUG_SUPPORTED)
		if (debugAvailable_) {
	...
	void Debug::SetObjectLabel(LabelTypes identifier, GLuint name, StringView label)
	{
#if defined(GL_DEBUG_SUPPORTED)
		if (debugAvailable_) {
	...
	void Debug::GetObjectLabel(LabelTypes identifier, GLuint name, GLsizei bufSize, GLsizei* length, char* label)
	{
#if defined(GL_DEBUG_SUPPORTED)
		if (debugAvailable_) {
