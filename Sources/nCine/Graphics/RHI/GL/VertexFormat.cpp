...
namespace nCine::RHI
{
	...
	bool VertexFormat::Attribute::operator==(const Attribute& other) const
	{
	...
	void VertexFormat::Attribute::Init(std::uint32_t index, GLint size, GLenum type)
	{
		enabled_ = true;
		vbo_ = nullptr;
		index_ = index;
	...
	void VertexFormat::Attribute::SetVboParameters(GLsizei stride, const GLvoid* pointer)
	{
