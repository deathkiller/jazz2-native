...
namespace nCine::RHI
{
	...
	Attribute::Attribute(GLuint program, GLuint index)
		: location_(-1), size_(0), type_(GL_FLOAT)
	{
		GLsizei length;
	...
	GLenum Attribute::GetBasicType() const
	{
		switch (type_) {
			case GL_FLOAT:
			case GL_FLOAT_VEC2:
	...
	std::int32_t Attribute::GetComponentCount() const
	{
		switch (type_) {
			case GL_BYTE:
	...
	bool Attribute::HasReservedPrefix() const
	{
