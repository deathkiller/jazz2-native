#include "LegacyGlUniformCache.h"
...
namespace nCine::RHI::LegacyGL
{
	...
	bool LegacyGlUniformCache::SetFloatValue(float v0)
	{
	...
	bool LegacyGlUniformCache::SetFloatValue(float v0, float v1)
	{
	...
	bool LegacyGlUniformCache::SetFloatValue(float v0, float v1, float v2)
	{
	...
	bool LegacyGlUniformCache::SetFloatValue(float v0, float v1, float v2, float v3)
	{
	...
	bool LegacyGlUniformCache::SetIntVector(const std::int32_t* vec)
	{
	...
	bool LegacyGlUniformCache::SetIntValue(std::int32_t v0)
	{
	...
	bool LegacyGlUniformCache::SetIntValue(std::int32_t v0, std::int32_t v1)
	{
	...
	bool LegacyGlUniformCache::SetIntValue(std::int32_t v0, std::int32_t v1, std::int32_t v2)
	{
	...
	bool LegacyGlUniformCache::SetIntValue(std::int32_t v0, std::int32_t v1, std::int32_t v2, std::int32_t v3)
	{
	...
	bool LegacyGlUniformCache::CommitValue()
	{
	...
	bool LegacyGlUniformCache::CheckFloat() const
	{
		return (_uniform != nullptr && _uniform->IsFloat());
	}

	bool LegacyGlUniformCache::CheckInt() const
	{
		return (_uniform != nullptr && !_uniform->IsFloat());
	}

	bool LegacyGlUniformCache::CheckComponents(std::uint32_t requiredComponents) const
	{
	...
	LegacyGlUniformBlockCache::LegacyGlUniformBlockCache(LegacyGlUniformBlock* uniformBlock)
		: _uniformBlock(uniformBlock), _dataPointer(nullptr), _usedSize(0)
	{
		...
		for (LegacyGlUniform& member : uniformBlock->_members) {
			_uniformCaches.push_back({member.GetName(), LegacyGlUniformCache(&member)});
			_uniformNameHashes.push_back(HashUniformName(member.GetName()));
		}
	}

	std::uint32_t LegacyGlUniformBlockCache::GetIndex() const
	{
		return (_uniformBlock != nullptr ? _uniformBlock->GetIndex() : 0);
	}

	std::int32_t LegacyGlUniformBlockCache::GetBindingIndex() const
	{
		return (_uniformBlock != nullptr ? _uniformBlock->GetBindingIndex() : 0);
	}

	std::int32_t LegacyGlUniformBlockCache::GetSize() const
	{
		return (_uniformBlock != nullptr ? _uniformBlock->GetSize() : 0);
	}

	std::uint8_t LegacyGlUniformBlockCache::GetAlignAmount() const
	{
		return (_uniformBlock != nullptr ? _uniformBlock->GetAlignAmount() : 0);
	}

	void LegacyGlUniformBlockCache::SetDataPointer(std::uint8_t* dataPointer)
	{
	...
	void LegacyGlUniformBlockCache::SetUsedSize(std::int32_t usedSize)
	{
		if (usedSize >= 0) {
			_usedSize = usedSize;
		}
	...
	bool LegacyGlUniformBlockCache::CopyData(std::uint32_t destIndex, const std::uint8_t* src, std::uint32_t numBytes)
	{
	...
	LegacyGlUniformCache* LegacyGlUniformBlockCache::GetUniform(StringView name)
	{
	...
	void LegacyGlUniformBlockCache::SetBlockBinding(std::int32_t blockBinding)
	{
