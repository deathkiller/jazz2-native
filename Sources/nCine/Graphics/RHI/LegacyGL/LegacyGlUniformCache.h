...
namespace nCine::RHI::LegacyGL
{
	/**
		...
		block, committing is a no-op — the whole block is published by @ref LegacyGlShaderUniformBlocks.
	*/
	class LegacyGlUniformCache
	{
	public:
		LegacyGlUniformCache()
			: _uniform(nullptr), _dataPointer(nullptr), _isDirty(false) {}
		explicit LegacyGlUniformCache(const LegacyGlUniform* uniform)
			: _uniform(uniform), _dataPointer(nullptr), _isDirty(false) {}

		inline const LegacyGlUniform* GetUniform() const {
			return _uniform;
		}
	...
	private:
		const LegacyGlUniform* _uniform;
	...
	/**
		...
		LegacyGlUniformCache for each member, each pointing at its member offset within that buffer. The block is
		uploaded as one contiguous range by @ref LegacyGlShaderUniformBlocks.
	*/
	class LegacyGlUniformBlockCache
	{
	public:
		LegacyGlUniformBlockCache()
			: _uniformBlock(nullptr), _dataPointer(nullptr), _usedSize(0) {}
		explicit LegacyGlUniformBlockCache(LegacyGlUniformBlock* uniformBlock);

		inline const LegacyGlUniformBlock* uniformBlock() const {
		...
		LegacyGlUniformCache* GetUniform(StringView name);
		void SetBlockBinding(std::int32_t blockBinding);

	private:
		LegacyGlUniformBlock* _uniformBlock;
		std::uint8_t* _dataPointer;
