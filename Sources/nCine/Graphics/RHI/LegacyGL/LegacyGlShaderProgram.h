...
#include "LegacyGlShaderTypes.h"
#include "LegacyGlVertexFormat.h"
#include "../RhiTypes.h"
...
namespace nCine::RHI::LegacyGL
{
	...
	/**
		@brief Shader program of the legacy GL backend (aliased as `RHI::ShaderProgram`)
	...
	*/
	...
	{
		friend class LegacyGlShaderUniforms;
		friend class LegacyGlShaderUniformBlocks;
	...
	public:
		...
		LegacyGlShaderProgram();
		explicit LegacyGlShaderProgram(QueryPhase queryPhase);
		LegacyGlShaderProgram(StringView vertexFile, StringView fragmentFile, Introspection introspection, QueryPhase queryPhase);
		LegacyGlShaderProgram(StringView vertexFile, StringView fragmentFile, Introspection introspection);
		LegacyGlShaderProgram(StringView vertexFile, StringView fragmentFile);
		~LegacyGlShaderProgram();

		LegacyGlShaderProgram(const LegacyGlShaderProgram&) = delete;
		LegacyGlShaderProgram& operator=(const LegacyGlShaderProgram&) = delete;
		...
		*/
		...
		LegacyGlVertexFormat::Attribute* GetAttribute(const char* name);

		inline void DefineVertexFormat(const LegacyGlBuffer* vbo) {
			DefineVertexFormat(vbo, nullptr, 0);
		}
		inline void DefineVertexFormat(const LegacyGlBuffer* vbo, const LegacyGlBuffer* ibo) {
			DefineVertexFormat(vbo, ibo, 0);
		}
		void DefineVertexFormat(const LegacyGlBuffer* vbo, const LegacyGlBuffer* ibo, std::uint32_t vboOffset);
		/** @brief Returns the vertex buffer last bound by @ref DefineVertexFormat(), or `nullptr` */
		inline const LegacyGlBuffer* GetBoundVbo() const {
			return _boundVbo;
		}
		...
		inline const LegacyGlBuffer* GetBoundIbo() const {
			return _boundIbo;
		}
		...
		// -- Backend extensions (used by the device and the effects) --
		...
		*/
		struct DispatchFacts
		{
			...
			const LegacyGlUniformBlock* InstanceBlock = nullptr;
		...
		const LegacyGlUniformBlock* FindBlock(const char* name) const;
	...
	private:
		...
		std::vector<LegacyGlUniform> _uniforms;
		std::vector<LegacyGlUniformBlock> _uniformBlocks;
		std::vector<LegacyGlAttribute> _attributes;
		...
		LegacyGlVertexFormat _vertexFormat;
		const LegacyGlBuffer* _boundVbo;
		std::uint32_t _boundVboOffset;
		const LegacyGlBuffer* _boundIbo;
