...
namespace nCine::RHI::LegacyGL
{
	...
	/**
		...
		Owns a @ref LegacyGlUniformCache for every active uniform of a program that is not part of a block,
	...
	*/
	...
	{
	public:
		// The legacy GL backend stores the caches in a flat container whose value type is the cache
		// itself (the name lives inside each cache's uniform), so range-iteration yields `LegacyGlUniformCache&`
		...
		using UniformHashMapType = std::vector<LegacyGlUniformCache>;

		LegacyGlShaderUniforms();
		explicit LegacyGlShaderUniforms(LegacyGlShaderProgram* shaderProgram);
		LegacyGlShaderUniforms(LegacyGlShaderProgram* shaderProgram, const char* includeOnly, const char* exclude);

		inline void SetProgram(LegacyGlShaderProgram* shaderProgram) {
			SetProgram(shaderProgram, nullptr, nullptr);
		}
		void SetProgram(LegacyGlShaderProgram* shaderProgram, const char* includeOnly, const char* exclude);
		...
		LegacyGlUniformCache* GetUniform(const char* name);
		/** @brief Returns the container of all managed uniform caches (range-iterates as `LegacyGlUniformCache&`) */
	...
	private:
		LegacyGlShaderProgram* _shaderProgram;
		bool _maybeDirty;
	...
	/**
		...
		Owns a @ref LegacyGlUniformBlockCache per active block, distributes a shared host data buffer across
	...
	*/
	...
	public:
		...
		// block), so range-iteration yields `LegacyGlUniformBlockCache&` directly, matching the OpenGL backend.
		using UniformHashMapType = std::vector<LegacyGlUniformBlockCache>;
		...
		explicit LegacyGlShaderUniformBlocks(LegacyGlShaderProgram* shaderProgram);
		LegacyGlShaderUniformBlocks(LegacyGlShaderProgram* shaderProgram, const char* includeOnly, const char* exclude);
		...
		LegacyGlUniformBlockCache* GetUniformBlock(const char* name);
		/** @brief Returns the container of all managed block caches (range-iterates as `LegacyGlUniformBlockCache&`) */
