...
{
	/**
		@brief Debug-output and object-labelling stub of the legacy GL backend

		Legacy OpenGL has no device-side debug facility, so every entry point is a no-op. The class exists only to
	...
	*/
	...
	{
	public:
		/** @brief Object types that can be labelled (values are irrelevant for the legacy GL backend) */
		...
		/**
			@brief RAII scope for a debug message group (a no-op for the legacy GL backend)
		*/
		class ScopedGroup
		{
		public:
