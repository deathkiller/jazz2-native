...
{
	std::uint32_t LegacyGlShaderProgram::_nextHandle = 1;

	LegacyGlShaderProgram::LegacyGlShaderProgram()
		: LegacyGlShaderProgram(QueryPhase::Immediate)
	{
	}

	LegacyGlShaderProgram::LegacyGlShaderProgram(QueryPhase queryPhase)
	...
	LegacyGlShaderProgram::LegacyGlShaderProgram(StringView vertexFile, StringView fragmentFile, Introspection introspection, QueryPhase queryPhase)
		: LegacyGlShaderProgram(queryPhase)
	{
	...
	LegacyGlShaderProgram::LegacyGlShaderProgram(StringView vertexFile, StringView fragmentFile, Introspection introspection)
		: LegacyGlShaderProgram(vertexFile, fragmentFile, introspection, QueryPhase::Immediate)
	{
	}

	LegacyGlShaderProgram::LegacyGlShaderProgram(StringView vertexFile, StringView fragmentFile)
		: LegacyGlShaderProgram(vertexFile, fragmentFile, Introspection::Enabled, QueryPhase::Immediate)
	{
	}
	...
	bool LegacyGlShaderProgram::IsLinked() const
	{
		return (_status == Status::Linked || _status == Status::LinkedWithDeferredQueries || _status == Status::LinkedWithIntrospection);
	}

	bool LegacyGlShaderProgram::AttachShaderFromFile(ShaderStage stage, StringView filename)
	{
	...
	bool LegacyGlShaderProgram::AttachShaderFromString(ShaderStage stage, StringView string)
	{
		static_cast<void>(stage);
	...
	bool LegacyGlShaderProgram::AttachShaderFromStrings(ShaderStage stage, ArrayView<const StringView> strings)
	{
		static_cast<void>(stage);
	...
	bool LegacyGlShaderProgram::AttachShaderFromStringsAndFile(ShaderStage stage, ArrayView<const StringView> strings, StringView filename)
	{
		static_cast<void>(stage);
		static_cast<void>(strings);
		static_cast<void>(filename);
	...
	bool LegacyGlShaderProgram::Link(Introspection introspection)
	{
		return FinalizeAfterLinking(introspection);
	}

	bool LegacyGlShaderProgram::FinalizeAfterLinking(Introspection introspection)
	{
	...
	void LegacyGlShaderProgram::Use()
	{
		LegacyGlDevice::BindProgram(this);
	}

	void LegacyGlShaderProgram::PerformIntrospection()
	{
		if (_introspection != Introspection::Disabled && _status != Status::LinkedWithIntrospection) {
			...
			_generatedEffect = (!_programName.empty()
				? LegacyGlDevice::FindGeneratedEffect(_programName.data(), _variantName.data()) : nullptr);
	...
	void LegacyGlShaderProgram::ImportReflection()
	{
		...
		for (std::size_t i = 0; i < reflection.BlockCount; i++) {
			...
			LegacyGlUniformBlock& block = _uniformBlocks.back();
	...
	bool LegacyGlShaderProgram::HasAttribute(const char* name) const
	{
		for (const LegacyGlAttribute& a : _attributes) {
	...
	LegacyGlVertexFormat::Attribute* LegacyGlShaderProgram::GetAttribute(const char* name)
	{
		for (const LegacyGlAttribute& a : _attributes) {
	...
	void LegacyGlShaderProgram::DefineVertexFormat(const LegacyGlBuffer* vbo, const LegacyGlBuffer* ibo, std::uint32_t vboOffset)
	{
		_boundVbo = vbo;
		...
		if (vbo != nullptr) {
			for (const LegacyGlAttribute& a : _attributes) {
				if (a.GetLocation() >= 0) {
					LegacyGlVertexFormat::Attribute& attr = _vertexFormat[std::uint32_t(a.GetLocation())];
					attr.setVbo(vbo);
	...
	void LegacyGlShaderProgram::Reset()
	{
		_uniforms.clear();
	...
	void LegacyGlShaderProgram::SetObjectLabel(StringView label)
	{
		// The label is kept for log messages only - effect identity comes from SetProgramIdentity()
		_label = label;
	}

	void LegacyGlShaderProgram::SetProgramIdentity(const char* programName, const char* variantName)
	{
	...
	const LegacyGlUniformBlock* LegacyGlShaderProgram::FindBlock(const char* name) const
	{
		for (const LegacyGlUniformBlock& block : _uniformBlocks) {
	...
	void LegacyGlShaderProgram::SetResolvedUniform(const char* name, const std::uint8_t* data)
	{
	...
	const std::uint8_t* LegacyGlShaderProgram::ResolveUniform(const char* name) const
	{
