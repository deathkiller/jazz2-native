...
{
	/**
		@brief Host-memory buffer object of the legacy GL backend (aliased as `RHI::Buffer`)

		The legacy GL backend consumes vertices and uniform blocks from host memory (GE draws are fed through a
		display list the CPU writes into, so there is no persistent device-side vertex store to upload
		into), which makes this class a plain resizable byte store exactly like the software backend's: @ref
	...
	*/
	...
	{
	public:
		explicit LegacyGlBuffer(BufferTarget target);
		~LegacyGlBuffer() = default;

		LegacyGlBuffer(const LegacyGlBuffer&) = delete;
		LegacyGlBuffer& operator=(const LegacyGlBuffer&) = delete;
		...
		/** @brief Sets a debug label for the buffer (ignored by the legacy GL backend) */
		...
		/** @brief Records the buffer handle bound for a target (inert for the legacy GL backend) */
		inline static void SetBoundHandle(std::uint32_t target, std::uint32_t glHandle) {
			static_cast<void>(target);
			static_cast<void>(glHandle);
		}
		/** @brief Marks a buffer handle as bound for a target (inert for the legacy GL backend) */
