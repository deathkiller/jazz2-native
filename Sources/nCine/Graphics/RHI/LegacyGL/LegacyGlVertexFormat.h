...
namespace nCine::RHI::LegacyGL
{
	...
	/**
		@brief Vertex layout description of the legacy GL backend (aliased as `RHI::VertexFormat`)

		Records the set of vertex attributes (component count, type, stride, offset and source buffer) the
		way the OpenGL backend does, so the pipeline's attribute setup code compiles unchanged. The legacy
		GL draw dispatch synthesizes sprite corners from the per-draw uniform block and submits general
		vertex data with a layout of its own, so the recorded layout is currently informational -
	...
	*/
	...
	public:
		...
		class Attribute
		{
			friend class LegacyGlVertexFormat;
		...
		public:
			...
			// needing the complete `LegacyGlBuffer` type in this header
			...
			inline const LegacyGlBuffer* GetVbo() const {
				return _vbo;
			}
			...
			inline void setVbo(const LegacyGlBuffer* vbo) {
				_vbo = vbo;
			}
		...
		inline const LegacyGlBuffer* GetIbo() const {
			return _ibo;
		}
		inline void SetIbo(const LegacyGlBuffer* ibo) {
			_ibo = ibo;
		}
		/** @brief Applies the vertex format (no-op for the legacy GL backend) */
		void Define() {}
		...
		bool operator==(const LegacyGlVertexFormat& other) const {
	...
	private:
		...
		const LegacyGlBuffer* _ibo;
	};

	/**
		@brief Vertex array object stub of the legacy GL backend (aliased as `RHI::VertexArray`)

		Legacy OpenGL has no vertex-array state object to capture, so this only satisfies the contract alias with
		inert bind/label operations.
	*/
	class LegacyGlVertexArray
	{
	public:
		LegacyGlVertexArray() = default;
		~LegacyGlVertexArray() = default;

		LegacyGlVertexArray(const LegacyGlVertexArray&) = delete;
		LegacyGlVertexArray& operator=(const LegacyGlVertexArray&) = delete;
