...
namespace nCine::RHI::LegacyGL
{
	...
	/**
		...
		Holds everything the legacy GL backend needs about one uniform imported from the offline
		...
		value storage — a @ref LegacyGlUniformCache holds the value.
	*/
	class LegacyGlUniform
	{
		friend class LegacyGlShaderProgram;
		friend class LegacyGlUniformBlock;
	...
	public:
		...
		LegacyGlUniform(LegacyGlShaderProgram* owner, const char* name, ShaderCompiler::UniformType type, std::int32_t arraySize, std::int32_t location)
		...
		inline LegacyGlShaderProgram* GetOwner() const {
			return _owner;
		}
	...
	/**
		...
		metadata of its member uniforms (each with a std140 offset). The legacy GL backend does not pad the
		size to any device alignment, so @ref GetAlignAmount() is always 0.
	*/
	class LegacyGlUniformBlock
	{
		friend class LegacyGlShaderProgram;
		friend class LegacyGlUniformBlockCache;
	...
	public:
		...
		LegacyGlUniformBlock()
			: _index(0), _size(0), _alignAmount(0), _bindingIndex(-1)
		{
			_name[0] = '\0';
		}
		LegacyGlUniformBlock(std::uint32_t index, const char* name, std::int32_t dataSize)
		...
		LegacyGlUniform* GetUniform(const char* name) {
			for (LegacyGlUniform& u : _members) {
	...
	private:
		...
		std::vector<LegacyGlUniform> _members;
	...
	*/
	...
	public:
		...
		LegacyGlAttribute(const char* name, ShaderCompiler::UniformType type, std::int32_t location)
