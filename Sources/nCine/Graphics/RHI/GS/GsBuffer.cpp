...
{
	std::uint32_t GsBuffer::_nextHandle = 1;

	GsBuffer::GsBuffer(BufferTarget target)
		: _handle(_nextHandle++), _target(target)
	{
	}

	bool GsBuffer::Bind() const
	{
	...
	bool GsBuffer::Unbind() const
	{
		return true;
	}

	void GsBuffer::BufferData(std::size_t size, const void* data, BufferUsage usage)
	{
	...
	void GsBuffer::BufferSubData(std::size_t offset, std::size_t size, const void* data)
	{
	...
	void GsBuffer::BufferStorage(std::size_t size, const void* data, MapFlags flags)
	{
	...
	void GsBuffer::BindBufferBase(std::uint32_t index)
	{
		BindBufferRange(index, 0, _storage.size());
	}

	void GsBuffer::BindBufferRange(std::uint32_t index, std::size_t offset, std::size_t size)
	{
		...
		GsDevice::BindUniformRange(index, _storage.data() + offset, std::uint32_t(size));
	}

	void* GsBuffer::MapBufferRange(std::size_t offset, std::size_t length, MapFlags access)
	{
	...
	void GsBuffer::FlushMappedBufferRange(std::size_t offset, std::size_t length)
	{
	...
	void GsBuffer::SetObjectLabel(StringView label)
	{
