...
namespace nCine::RHI::GS
{
	...
	/**
		...
		Holds everything the GS backend needs about one uniform imported from the offline reflection: its name,
		scalar/vector/matrix type, array size, a synthetic sequential location and, for uniforms living inside
		a block, the owning block index and std140 byte offset. It carries no value storage - a
		@ref GsUniformCache holds the value.
	*/
	class GsUniform
	{
		friend class GsShaderProgram;
		friend class GsUniformBlock;
	...
	public:
		...
		GsUniform(GsShaderProgram* owner, const char* name, ShaderCompiler::UniformType type, std::int32_t arraySize, std::int32_t location)
		...
		inline GsShaderProgram* GetOwner() const {
			return _owner;
		}
	...
	/**
		...
		metadata of its member uniforms (each with a std140 offset). The GS backend does not pad the size to
		any device alignment - the blocks are plain host memory the draw dispatch decodes - so
		@ref GetAlignAmount() is always 0.
	*/
	class GsUniformBlock
	{
		friend class GsShaderProgram;
		friend class GsUniformBlockCache;
	...
	public:
		...
		GsUniformBlock(std::uint32_t index, const char* name, std::int32_t dataSize)
		...
		GsUniform* GetUniform(const char* name) {
			for (GsUniform& u : _members) {
	...
	*/
	...
	public:
		...
		GsAttribute(const char* name, ShaderCompiler::UniformType type, std::int32_t location)
