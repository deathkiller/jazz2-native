...
namespace nCine::RHI::GS
{
	...
	bool GsUniformCache::SetFloatValue(float v0)
	{
	...
	bool GsUniformCache::SetFloatValue(float v0, float v1)
	{
	...
	bool GsUniformCache::SetFloatValue(float v0, float v1, float v2)
	{
	...
	bool GsUniformCache::SetFloatValue(float v0, float v1, float v2, float v3)
	{
	...
	bool GsUniformCache::SetIntVector(const std::int32_t* vec)
	{
	...
	bool GsUniformCache::SetIntValue(std::int32_t v0)
	{
	...
	bool GsUniformCache::SetIntValue(std::int32_t v0, std::int32_t v1)
	{
	...
	bool GsUniformCache::SetIntValue(std::int32_t v0, std::int32_t v1, std::int32_t v2)
	{
	...
	bool GsUniformCache::SetIntValue(std::int32_t v0, std::int32_t v1, std::int32_t v2, std::int32_t v3)
	{
	...
	bool GsUniformCache::CommitValue()
	{
	...
	bool GsUniformCache::CheckFloat() const
	{
		return (_uniform != nullptr && _uniform->IsFloat());
	}

	bool GsUniformCache::CheckInt() const
	{
		return (_uniform != nullptr && !_uniform->IsFloat());
	}

	bool GsUniformCache::CheckComponents(std::uint32_t requiredComponents) const
	{
	...
	GsUniformBlockCache::GsUniformBlockCache(GsUniformBlock* uniformBlock)
		: _uniformBlock(uniformBlock), _dataPointer(nullptr), _usedSize(0)
	{
		...
		for (GsUniform& member : uniformBlock->_members) {
			_uniformCaches.push_back({member.GetName(), GsUniformCache(&member)});
			_uniformNameHashes.push_back(HashUniformName(member.GetName()));
		}
	}

	std::uint32_t GsUniformBlockCache::GetIndex() const
	{
		return (_uniformBlock != nullptr ? _uniformBlock->GetIndex() : 0);
	}

	std::int32_t GsUniformBlockCache::GetBindingIndex() const
	{
		return (_uniformBlock != nullptr ? _uniformBlock->GetBindingIndex() : 0);
	}

	std::int32_t GsUniformBlockCache::GetSize() const
	{
		return (_uniformBlock != nullptr ? _uniformBlock->GetSize() : 0);
	}

	std::uint8_t GsUniformBlockCache::GetAlignAmount() const
	{
		return (_uniformBlock != nullptr ? _uniformBlock->GetAlignAmount() : 0);
	}

	void GsUniformBlockCache::SetDataPointer(std::uint8_t* dataPointer)
	{
	...
	void GsUniformBlockCache::SetUsedSize(std::int32_t usedSize)
	{
		if (usedSize >= 0) {
			_usedSize = usedSize;
		}
	...
	bool GsUniformBlockCache::CopyData(std::uint32_t destIndex, const std::uint8_t* src, std::uint32_t numBytes)
	{
	...
	GsUniformCache* GsUniformBlockCache::GetUniform(StringView name)
	{
	...
	void GsUniformBlockCache::SetBlockBinding(std::int32_t blockBinding)
	{
