...
namespace nCine::RHI::GS
{
	...
	/**
		@brief Shader program of the GS backend (aliased as `RHI::ShaderProgram`)

		Does not compile or link GLSL - the Graphics Synthesizer has no programmable stage. Instead it carries
		the offline ShaderCompiler reflection (set with @ref SetReflection() exactly like the OpenGL backend)
		from which it imports uniforms, uniform blocks and attributes, and the true (program, variant) identity
		plumbed by the loaders with @ref SetProgramIdentity(), from which the generated fixed-function effect of
		the program is resolved (see @ref GetGeneratedEffect()) - the object label is kept for logging only.
		@ref Use() records the program as current on the device; committed loose-uniform values are published
		back here so the effect can read them by name.
	*/
	class GsShaderProgram
	{
		friend class GsShaderUniforms;
		friend class GsShaderUniformBlocks;
	...
	public:
		...
		GsShaderProgram();
		explicit GsShaderProgram(QueryPhase queryPhase);
		GsShaderProgram(StringView vertexFile, StringView fragmentFile, Introspection introspection, QueryPhase queryPhase);
		GsShaderProgram(StringView vertexFile, StringView fragmentFile, Introspection introspection);
		GsShaderProgram(StringView vertexFile, StringView fragmentFile);
		~GsShaderProgram();

		GsShaderProgram(const GsShaderProgram&) = delete;
		GsShaderProgram& operator=(const GsShaderProgram&) = delete;
		...
		/**
			...
			@p programName is the `.shader` program name and @p variantName the variant define it was compiled
			with (empty/null for the base variant) - the exact key of the generated fixed-function tables,
			plumbed by ContentResolver and RenderResources. Set it together with @ref SetReflection(), before
			@ref Link(); the table lookup runs at link time.
		*/
		...
		GsVertexFormat::Attribute* GetAttribute(const char* name);

		inline void DefineVertexFormat(const GsBuffer* vbo) {
			DefineVertexFormat(vbo, nullptr, 0);
		}
		inline void DefineVertexFormat(const GsBuffer* vbo, const GsBuffer* ibo) {
			DefineVertexFormat(vbo, ibo, 0);
		}
		void DefineVertexFormat(const GsBuffer* vbo, const GsBuffer* ibo, std::uint32_t vboOffset);
		/** @brief Returns the vertex buffer last bound by @ref DefineVertexFormat(), or `nullptr` */
		inline const GsBuffer* GetBoundVbo() const {
			return _boundVbo;
		}
		...
		inline const GsBuffer* GetBoundIbo() const {
			return _boundIbo;
		}
		...
		// -- GS backend extensions (used by the device and the effects) --
		...
		/**
			...
			@ref SetProgramIdentity() against the table the ShaderCompiler transpiled from the shaders'
			`fixed_function` blocks. The dispatch runs its function (or the backend pipeline stage its
			`pipeline` intrinsic names); a null entry is skipped with a one-time warning.
		*/
		...
		*/
		struct DispatchFacts
		{
			...
			const GsUniformBlock* InstanceBlock = nullptr;
		...
		const GsUniformBlock* FindBlock(const char* name) const;
	...
	private:
		...
		std::vector<GsUniform> _uniforms;
		std::vector<GsUniformBlock> _uniformBlocks;
		std::vector<GsAttribute> _attributes;
		...
		// The generated-table entry of this (program, variant) (see GetGeneratedEffect()); null when the
		// shader has no fixed_function block (the draw is then skipped with a one-time warning)
		const FixedFunctionGeneratedEffect* _generatedEffect;
		bool _unsupportedWarned = false;
		bool _ditherVariant;
		bool _usesPalette;
		DispatchFacts _dispatchFacts;
		// The true identity set by SetProgramIdentity(): the .shader program name and the variant define -
		// the generated-table key. Copies, because runtime loaders hand in transient strings.
		String _programName;
		String _variantName;
		// The shader name the program was tagged with (kept for log messages only)
		String _label;

		GsVertexFormat _vertexFormat;
		const GsBuffer* _boundVbo;
		std::uint32_t _boundVboOffset;
		const GsBuffer* _boundIbo;
