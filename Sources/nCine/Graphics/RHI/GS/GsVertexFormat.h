...
namespace nCine::RHI::GS
{
	...
	/**
		@brief Vertex layout description of the GS backend (aliased as `RHI::VertexFormat`)

		Records the set of vertex attributes (component count, type, stride, offset and source buffer) the way
		the OpenGL backend does, so the pipeline's attribute setup code compiles unchanged. The GS draw
		dispatch synthesizes sprite corners from the per-draw uniform block and writes general vertex data
		into GIF packets with its own layout, so the recorded layout is informational - @ref Define() and
		@ref Reset() have nothing to apply.
	*/
	...
	public:
		...
		public:
			...
			// needing the complete `GsBuffer` type in this header
			...
			inline const GsBuffer* GetVbo() const {
				return _vbo;
			}
			...
			inline void setVbo(const GsBuffer* vbo) {
				_vbo = vbo;
			}
		...
		inline const GsBuffer* GetIbo() const {
			return _ibo;
		}
		inline void SetIbo(const GsBuffer* ibo) {
			_ibo = ibo;
		}
		/** @brief Applies the vertex format (no-op for the GS backend) */
		void Define() {}
		...
		bool operator==(const GsVertexFormat& other) const {
	...
	/**
		@brief Vertex array object stub of the GS backend (aliased as `RHI::VertexArray`)

		The GS has no vertex-array state object to capture - a draw is a GIF packet built from scratch - so
		this only satisfies the contract alias with inert bind/label operations.
	*/
	class GsVertexArray
	{
	public:
		GsVertexArray() = default;
		~GsVertexArray() = default;

		GsVertexArray(const GsVertexArray&) = delete;
		GsVertexArray& operator=(const GsVertexArray&) = delete;
