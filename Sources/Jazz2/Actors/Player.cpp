...
namespace Jazz2::Actors
{
	...
	void Player::ReceiveLevelCarryOver(ExitType exitType, const PlayerCarryOver& carryOver)
	{
		...
		// Preload all weapons
		for (std::int32_t i = 0; i < arraySize<std::int32_t>(_weaponAmmo); i++) {
			if (_weaponAmmo[i] != 0) {
	...
	void Player::InitializeFromStream(ILevelHandler* levelHandler, Stream& src)
	{
		...
		RETURN_ASSERT_MSG(weaponCount == arraySize<std::int32_t>(_weaponAmmoCheckpoint), "Weapon count mismatch");
	...
	void Player::SerializeResumableToStream(Stream& dest)
	{
		...
		dest.WriteVariableInt32(arraySize<std::int32_t>(_weaponAmmoCheckpoint));
