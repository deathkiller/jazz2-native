...
namespace Jazz2::UI::Menu
{
	...
	void GameplayOptionsSection::OnDraw(Canvas* canvas)
	{
		...
		_root->DrawStringShadow(_("Gameplay"), charOffset, center.X, topLine - 21.0f, IMenuContainer::FontLayer,
			Alignment::Center, Colorf(0.46f, 0.46f, 0.46f, 0.5f), 0.9f, 0.7f, 1.1f, 1.1f, 0.4f, 0.9f);
		...
		for (int i = 0; i < (int)Item::Count; i++) {
			...
			if (i >= 2 && i <= lastBoolOption) {
				...
				if (i == (int)Item::AllowUnsignedScripts && enabled) {
					_root->DrawStringShadow(_("Enabled"), charOffset, center.X + 12.0f, center.Y + 22.0f, IMenuContainer::FontLayer - 10,
