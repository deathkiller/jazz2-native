...
namespace Jazz2::Compatibility
{
	...
	ConversionResult EventConverter::TryConvert(JJ2Level* level, JJ2Event old, uint32_t eventParams) const
	{
	...
	{
		return [ev](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			return { ev };
		};
	}

	EventConverter::ConversionFunction EventConverter::ConstantParamList(EventType ev, const std::array<uint8_t, 16>& eventParams)
	{
		return [ev, eventParams](JJ2Level* level, uint32_t jj2Params) mutable -> ConversionResult {
	...
	EventConverter::ConversionFunction EventConverter::ParamIntToParamList(EventType ev, const std::array<Pair<int, int>, 6>& paramDefs)
	{
	...
	void EventConverter::ConvertParamInt(uint32_t paramInt, const ArrayView<const Pair<int, int>>& paramTypes, uint8_t eventParams[16])
	{
		...
		for (auto& param : paramTypes) {
			...
			switch (param.first()) {
				case JJ2ParamBool: {
					eventParams[i++] = (uint8_t)(paramInt & 1);
					paramInt = paramInt >> 1;
					break;
				}
				case JJ2ParamUInt: {
				...
				case JJ2ParamInt: {
					...
					int highestBitValue = (1 << (param.second() - 1));
	...
	void EventConverter::AddDefaultConverters()
	{
		...
		Add(JJ2Event::SCENERY_COLLAPSE, [](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			...
			uint16_t waitTime = *(uint16_t*)&eventParams[0] * 25;
			return { EventType::SceneryCollapse, { (uint8_t)(waitTime & 0xff), (uint8_t)((waitTime >> 8) & 0xff), eventParams[1] } };
		});
		...
		Add(JJ2Event::MODIFIER_TUBE, [](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			uint8_t eventParams[16];
			ConvertParamInt(jj2Params, {
		...
		Add(JJ2Event::MODIFIER_SLIDE, [](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			uint8_t eventParams[16];
			ConvertParamInt(jj2Params, {
				{ JJ2ParamUInt, 2 }
		...
		Add(JJ2Event::MODIFIER_BELT_LEFT, [](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			...
			if (jj2Params == 0) {
				...
				left = 0;
				right = (uint8_t)(256 - jj2Params);
			} else {
				left = (uint8_t)jj2Params;
				right = 0;
			}

			return { EventType::AreaHForce, { left, right } };
		});
		Add(JJ2Event::MODIFIER_BELT_RIGHT, [](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			// TODO: Use single variable
			uint8_t left, right;
			if (jj2Params == 0) {
				left = 0;
				...
				left = (uint8_t)(256 - jj2Params);
				right = 0;
			} else {
				left = 0;
				right = (uint8_t)jj2Params;
			}

			return { EventType::AreaHForce, { left, right } };
		});
		Add(JJ2Event::MODIFIER_ACC_BELT_LEFT, [](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			if (jj2Params == 0) {
				jj2Params = 3;
			}

			return { EventType::AreaHForce, { 0, 0, (uint8_t)jj2Params } };
		});
		Add(JJ2Event::MODIFIER_ACC_BELT_RIGHT, [](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			if (jj2Params == 0) {
				jj2Params = 3;
			}

			return { EventType::AreaHForce, { 0, 0, 0, (uint8_t)jj2Params } };
		});

		Add(JJ2Event::MODIFIER_WIND_LEFT, [](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			// TODO: Use single variable
			uint8_t left, right;
			if (jj2Params > 127) {
				left = (uint8_t)(256 - jj2Params);
		...
		Add(JJ2Event::MODIFIER_WIND_RIGHT, [](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			return { EventType::AreaHForce, { 0, 0, 0, 0, 0, (uint8_t)jj2Params } };
		});

		Add(JJ2Event::MODIFIER_SET_WATER, [](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			uint8_t eventParams[16];
			ConvertParamInt(jj2Params, {
			...
			return { EventType::ModifierSetWater, { (uint8_t)(waterLevel & 0xff), (uint8_t)((waterLevel >> 8) & 0xff), eventParams[1], eventParams[2] } };
		});
		...
		Add(JJ2Event::AREA_LIMIT_X_SCROLL, [](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			...
			return { EventType::ModifierLimitCameraView, { (uint8_t)(left & 0xff), (uint8_t)((left >> 8) & 0xff), (uint8_t)(width & 0xff), (uint8_t)((width >> 8) & 0xff) } };
		});
		...
		Add(JJ2Event::AREA_EOL_WARP, [](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			uint8_t eventParams[16];
			ConvertParamInt(jj2Params, {
		...
		Add(JJ2Event::AREA_SECRET_WARP, [](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			uint8_t eventParams[16];
			ConvertParamInt(jj2Params, {
		...
		Add(JJ2Event::EOL_SIGN, [](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			...
			return { EventType::SignEOL, { (uint8_t)(eventParams[0] == 1 ? 4 : 1) } };
		});

		Add(JJ2Event::BONUS_SIGN, ConstantParamList(EventType::AreaEndOfLevel, { 3, 0, 0, 0, 0 }));

		Add(JJ2Event::AREA_TEXT, [](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			uint8_t eventParams[16];
			ConvertParamInt(jj2Params, {
				{ JJ2ParamUInt, 8 },	// Text
		...
		Add(JJ2Event::AREA_NO_FIRE, [](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			uint8_t eventParams[16];
			ConvertParamInt(jj2Params, {
		...
		Add(JJ2Event::SNOW, [](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			uint8_t eventParams[16];
			ConvertParamInt(jj2Params, {
		...
		// Triggers events
		Add(JJ2Event::TRIGGER_CRATE, [](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			uint8_t eventParams[16];
			ConvertParamInt(jj2Params, {
			...
			return { EventType::TriggerCrate, { eventParams[0], (uint8_t)(eventParams[1] == 0 ? 1 : 0), eventParams[2] } };
		});
		...
		// Warp events
		Add(JJ2Event::WARP_ORIGIN, [](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			uint8_t eventParams[16];
			ConvertParamInt(jj2Params, {
		...
		Add(JJ2Event::LIGHT_STEADY, [](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			...
			switch (eventParams[0]) {
				default:
				case 0: { // Normal
					uint16_t radiusNear = (uint16_t)(eventParams[1] == 0 ? 60 : eventParams[1] * 6);
					uint16_t radiusFar = (uint16_t)(radiusNear * 1.666f);
					return { EventType::LightSteady, { 255, 10, (uint8_t)(radiusNear & 0xff), (uint8_t)((radiusNear >> 8) & 0xff), (uint8_t)(radiusFar & 0xff), (uint8_t)((radiusFar >> 8) & 0xff) } };
				}
				...
				case 3: { // Flicker light
					uint16_t radiusNear = (uint16_t)(eventParams[1] == 0 ? 60 : eventParams[1] * 6);
					uint16_t radiusFar = (uint16_t)(radiusNear * 1.666f);
					return { EventType::LightFlicker, { std::min((uint8_t)(110 + eventParams[1] * 2), (uint8_t)255), 40, (uint8_t)(radiusNear & 0xff), (uint8_t)((radiusNear >> 8) & 0xff), (uint8_t)(radiusFar & 0xff), (uint8_t)((radiusFar >> 8) & 0xff) } };
				}

				case 4: { // Bright normal light
					uint16_t radiusNear = (uint16_t)(eventParams[1] == 0 ? 80 : eventParams[1] * 7);
					uint16_t radiusFar = (uint16_t)(radiusNear * 1.25f);
					return { EventType::LightSteady, { 255, 200, (uint8_t)(radiusNear & 0xff), (uint8_t)((radiusNear >> 8) & 0xff), (uint8_t)(radiusFar & 0xff), (uint8_t)((radiusFar >> 8) & 0xff) } };
				}

				case 5: { // Laser shield/Illuminate Surroundings
					return { EventType::LightIlluminate, { (uint8_t)(eventParams[1] < 1 ? 1 : eventParams[1]) } };
				}
		...
		Add(JJ2Event::LIGHT_PULSE, [](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			uint8_t eventParams[16];
			ConvertParamInt(jj2Params, {
			...
			uint16_t radiusNear1 = (uint16_t)(eventParams[3] == 0 ? 20 : eventParams[3] * 4.8f);
			uint16_t radiusNear2 = (uint16_t)(radiusNear1 * 2);
			uint16_t radiusFar = (uint16_t)(radiusNear1 * 2.4f);

			uint8_t speed = (uint8_t)(eventParams[0] == 0 ? 6 : eventParams[0]); // Quickfix for Tube2.j2l to look better
			uint8_t sync = eventParams[1];

			switch (eventParams[2]) {
				default:
				case 0: { // Normal
					return { EventType::LightPulse, { 255, 10, (uint8_t)(radiusNear1 & 0xff), (uint8_t)((radiusNear1 >> 8) & 0xff), (uint8_t)(radiusNear2 & 0xff), (uint8_t)((radiusNear2 >> 8) & 0xff), (uint8_t)(radiusFar & 0xff), (uint8_t)((radiusFar >> 8) & 0xff), speed, sync } };
				}
				case 4: { // Bright normal light
					return { EventType::LightPulse, { 255, 200, (uint8_t)(radiusNear1 & 0xff), (uint8_t)((radiusNear1 >> 8) & 0xff), (uint8_t)(radiusNear2 & 0xff), (uint8_t)((radiusNear2 >> 8) & 0xff), (uint8_t)(radiusFar & 0xff), (uint8_t)((radiusFar >> 8) & 0xff), speed, sync } };
				}
				case 5: { // Laser shield/Illuminate Surroundings
		...
		Add(JJ2Event::BOLL_SPIKE_3D, [](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			uint8_t eventParams[16];
			ConvertParamInt(jj2Params, {
				{ JJ2ParamUInt, 2 },	// Sync
				{ JJ2ParamInt, 6 },		// Speed
		...
		Add(JJ2Event::BRIDGE, [](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			...
			uint16_t width = (eventParams[0] * 2);
			return { EventType::Bridge, { (uint8_t)(width & 0xff), (uint8_t)((width >> 8) & 0xff), eventParams[1], eventParams[2] } };
		});
		...
		Add(JJ2Event::BOSS_DEVAN_ROBOT, [](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			uint8_t eventParams[16];
			ConvertParamInt(jj2Params, {
		...
		Add(JJ2Event::GEM_RING, [](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			uint8_t eventParams[16];
			ConvertParamInt(jj2Params, {
		...
		Add(JJ2Event::AMMO_BOUNCER, ConstantParamList(EventType::Ammo, { (uint8_t)WeaponType::Bouncer }));
		Add(JJ2Event::AMMO_FREEZER, ConstantParamList(EventType::Ammo, { (uint8_t)WeaponType::Freezer }));
		Add(JJ2Event::AMMO_SEEKER, ConstantParamList(EventType::Ammo, { (uint8_t)WeaponType::Seeker }));
		Add(JJ2Event::AMMO_RF, ConstantParamList(EventType::Ammo, { (uint8_t)WeaponType::RF }));
		Add(JJ2Event::AMMO_TOASTER, ConstantParamList(EventType::Ammo, { (uint8_t)WeaponType::Toaster }));
		Add(JJ2Event::AMMO_TNT, ConstantParamList(EventType::Ammo, { (uint8_t)WeaponType::TNT }));
		Add(JJ2Event::AMMO_PEPPER, ConstantParamList(EventType::Ammo, { (uint8_t)WeaponType::Pepper }));
		Add(JJ2Event::AMMO_ELECTRO, ConstantParamList(EventType::Ammo, { (uint8_t)WeaponType::Electro }));

		Add(JJ2Event::FAST_FIRE, NoParamList(EventType::FastFire));
		Add(JJ2Event::POWERUP_BLASTER, ConstantParamList(EventType::PowerUpWeapon, { (uint8_t)WeaponType::Blaster }));
		Add(JJ2Event::POWERUP_BOUNCER, ConstantParamList(EventType::PowerUpWeapon, { (uint8_t)WeaponType::Bouncer }));
		Add(JJ2Event::POWERUP_FREEZER, ConstantParamList(EventType::PowerUpWeapon, { (uint8_t)WeaponType::Freezer }));
		Add(JJ2Event::POWERUP_SEEKER, ConstantParamList(EventType::PowerUpWeapon, { (uint8_t)WeaponType::Seeker }));
		Add(JJ2Event::POWERUP_RF, ConstantParamList(EventType::PowerUpWeapon, { (uint8_t)WeaponType::RF }));
		Add(JJ2Event::POWERUP_TOASTER, ConstantParamList(EventType::PowerUpWeapon, { (uint8_t)WeaponType::Toaster }));
		Add(JJ2Event::POWERUP_TNT, ConstantParamList(EventType::PowerUpWeapon, { (uint8_t)WeaponType::TNT }));
		Add(JJ2Event::POWERUP_PEPPER, ConstantParamList(EventType::PowerUpWeapon, { (uint8_t)WeaponType::Pepper }));
		Add(JJ2Event::POWERUP_ELECTRO, ConstantParamList(EventType::PowerUpWeapon, { (uint8_t)WeaponType::Electro }));

		Add(JJ2Event::FOOD_APPLE, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Apple }));
		Add(JJ2Event::FOOD_BANANA, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Banana }));
		Add(JJ2Event::FOOD_CHERRY, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Cherry }));
		Add(JJ2Event::FOOD_ORANGE, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Orange }));
		Add(JJ2Event::FOOD_PEAR, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Pear }));
		Add(JJ2Event::FOOD_PRETZEL, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Pretzel }));
		Add(JJ2Event::FOOD_STRAWBERRY, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Strawberry }));
		Add(JJ2Event::FOOD_LEMON, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Lemon }));
		Add(JJ2Event::FOOD_LIME, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Lime }));
		Add(JJ2Event::FOOD_THING, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Thing }));
		Add(JJ2Event::FOOD_WATERMELON, ConstantParamList(EventType::Food, { (uint8_t)FoodType::WaterMelon }));
		Add(JJ2Event::FOOD_PEACH, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Peach }));
		Add(JJ2Event::FOOD_GRAPES, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Grapes }));
		Add(JJ2Event::FOOD_LETTUCE, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Lettuce }));
		Add(JJ2Event::FOOD_EGGPLANT, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Eggplant }));
		Add(JJ2Event::FOOD_CUCUMBER, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Cucumber }));
		Add(JJ2Event::FOOD_PEPSI, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Pepsi }));
		Add(JJ2Event::FOOD_COKE, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Coke }));
		Add(JJ2Event::FOOD_MILK, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Milk }));
		Add(JJ2Event::FOOD_PIE, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Pie }));
		Add(JJ2Event::FOOD_CAKE, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Cake }));
		Add(JJ2Event::FOOD_DONUT, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Donut }));
		Add(JJ2Event::FOOD_CUPCAKE, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Cupcake }));
		Add(JJ2Event::FOOD_CHIPS, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Chips }));
		Add(JJ2Event::FOOD_CANDY, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Candy }));
		Add(JJ2Event::FOOD_CHOCOLATE, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Chocolate }));
		Add(JJ2Event::FOOD_ICE_CREAM, ConstantParamList(EventType::Food, { (uint8_t)FoodType::IceCream }));
		Add(JJ2Event::FOOD_BURGER, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Burger }));
		Add(JJ2Event::FOOD_PIZZA, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Pizza }));
		Add(JJ2Event::FOOD_FRIES, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Fries }));
		Add(JJ2Event::FOOD_CHICKEN_LEG, ConstantParamList(EventType::Food, { (uint8_t)FoodType::ChickenLeg }));
		Add(JJ2Event::FOOD_SANDWICH, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Sandwich }));
		Add(JJ2Event::FOOD_TACO, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Taco }));
		Add(JJ2Event::FOOD_HOT_DOG, ConstantParamList(EventType::Food, { (uint8_t)FoodType::HotDog }));
		Add(JJ2Event::FOOD_HAM, ConstantParamList(EventType::Food, { (uint8_t)FoodType::Ham }));
		...
		Add(JJ2Event::CRATE_BOMB, [](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			...
			if (eventParams[0] > 0 && eventParams[1] > 0) {
				...
				return { EventType::Crate, { (uint8_t)EventType::Bomb, (uint8_t)((int32_t)EventType::Bomb >> 8), 1 } };
			} else {
				return { EventType::Crate };
			}
		});
	...
	EventConverter::ConversionFunction EventConverter::GetSpringConverter(uint8_t type, bool horizontal, bool frozen)
	{
		return [type, horizontal, frozen](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			uint8_t eventParams[16];
			ConvertParamInt(jj2Params, {
			...
			return { EventType::Spring, { type, (uint8_t)(horizontal ? (eventParams[4] != 0 ? 5 : 4) : eventParams[0] * 2), eventParams[1], eventParams[2], eventParams[3], (uint8_t)(frozen ? 1 : 0) } };
		};
	}

	EventConverter::ConversionFunction EventConverter::GetPlatformConverter(uint8_t type)
	{
		return [type](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			uint8_t eventParams[16];
			ConvertParamInt(jj2Params, {
				{ JJ2ParamUInt, 2 },	// Sync
				{ JJ2ParamInt, 6 },		// Speed
	...
	EventConverter::ConversionFunction EventConverter::GetPoleConverter(uint8_t theme)
	{
		return [theme](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			uint8_t eventParams[16];
			ConvertParamInt(jj2Params, {
			...
			constexpr int AdjustX = 2;
			constexpr int AdjustY = 2;

			uint16_t x = (uint16_t)((int8_t)eventParams[1] + 16 - AdjustX);
			uint16_t y = (uint16_t)((eventParams[0] == 0 ? 24 : eventParams[0]) - AdjustY);
			return { EventType::Pole, { theme, (uint8_t)(x & 0xff), (uint8_t)((x >> 8) & 0xff), (uint8_t)(y & 0xff), (uint8_t)((y >> 8) & 0xff) } };
		};
	}

	EventConverter::ConversionFunction EventConverter::GetAmmoCrateConverter(uint8_t type)
	{
		return [type](JJ2Level* level, uint32_t jj2Params) -> ConversionResult {
			return { EventType::CrateAmmo, { type } };
		};
	...
	EventConverter::ConversionFunction EventConverter::GetBossConverter(EventType ev, uint8_t customParam)
	{
