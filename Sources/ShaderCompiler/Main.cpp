...
namespace
{
	...
	void PrintUsage()
	{
		std::fprintf(stderr,
			...
			"  --emit-fixed-function <pvr|gx> <output.h> <input.shader ...> Transpile fixed_function blocks into a per-backend aggregate header\n");
	}
	...
	*/
	...
	/** Builds the aggregate "PvrGeneratedEffects.h" / "GxGeneratedEffects.h" contents from the transpiled effects */
	...
	{
		const char* backendArg = (backend == FixedFunctionBackend::Pvr ? "pvr" : "gx");
		const char* guard = (backend == FixedFunctionBackend::Pvr ? "WITH_RHI_PVR" : "WITH_RHI_GX");
		const char* ns = (backend == FixedFunctionBackend::Pvr ? "nCine::RHI::PVR" : "nCine::RHI::GX");
	...
	/**
		...
		C++ and writes one per-backend aggregate header. Backend selection: a `pvr`/`gx` block overrides the
		generic fixed_function block for that backend; programs with no applicable block are simply
		absent from the emitted table. Unlike the software transpiler (which declines unsupported shaders),
		any error inside a block is FATAL, reported as "<file>:<line>: error: ..." — the block is authored intent.
	*/
	int RunEmitFixedFunction(const char* backendName, const char* outputPath, char** inputPaths, int inputCount)
	{
		...
		if (StringView(backendName) == "pvr") {
			...
			std::fprintf(stderr, "error: unknown fixed-function backend \"%s\" (expected pvr or gx)\n", backendName);
			return 2;
		}
		const FixedFunctionTarget overrideTarget =
			(backend == FixedFunctionBackend::Pvr ? FixedFunctionTarget::Pvr : FixedFunctionTarget::Gx);
		...
		for (int fi = 0; fi < inputCount; fi++) {
			...
			for (const ShaderDocument& document : documents) {
				// A pvr/gx block overrides the generic fixed_function block for its backend
				const FixedFunctionBlock* block = nullptr;
				const char* targetName = "";
				for (const FixedFunctionBlock& b : document.FixedFunctionBlocks) {
					if (b.Target == overrideTarget) {
						block = &b;
						targetName = backendName;
						break;
					}
					if (b.Target == FixedFunctionTarget::Generic && block == nullptr) {
						block = &b;
					}
				}
				if (block == nullptr) {
				...
				String provenance = BaseNameOf(inputPath) + ":fixed_function("_s + targetName + ")"_s;
...
int main(int argc, char* argv[])
{
	...
	//   ShaderCompiler --emit-fixed-function <pvr|gx> <output.h> <input1.shader> [input2.shader ...]
	if (argc >= 2 && StringView(argv[1]) == "--emit-fixed-function") {
		if (argc < 5) {
			std::fprintf(stderr, "error: --emit-fixed-function requires <pvr|gx>, <output.h> and at least one input .shader\n");
			return 2;
		}
