...
//###==##====#=====--==~--~=~- --- -- -  -  -   -
	...
	std::int32_t Lzma2Stream::ReadInternal(void* ptr, std::int32_t size)
	{
		...
		// Whatever this call already decompressed is kept even when it then fails: liblzma writes its output
		// before it reports an error in the input that followed, and returning the failure straight away would
		// discard those bytes. The error is reported on the next call instead, because the state stays Failed
		...
		if (res != LZMA_OK && res != LZMA_STREAM_END) {
			...
			if (produced <= 0) {
				...
				LOGD("LZMA2 stream ended prematurely with error {} after {} bytes", (std::int32_t)res, produced);
			}
#endif
