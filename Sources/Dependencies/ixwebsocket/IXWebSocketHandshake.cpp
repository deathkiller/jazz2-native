...
namespace ix
{
    ...
    WebSocketInitResult WebSocketHandshake::serverHandshake(
        ...
        const ProtocolSelectionCallback& protocolSelectionCallback,
        HttpRequestPtr request)
    {
        ...
        // Parse sub-protocols requested by the client
        std::vector<std::string> requestedProtocols;
        if (headers.find("sec-websocket-protocol") != headers.end())
        {
            std::istringstream protocolStream(headers["sec-websocket-protocol"]);
            std::string token;
            while (std::getline(protocolStream, token, ','))
            {
                size_t start = token.find_first_not_of(" \t");
                size_t end   = token.find_last_not_of(" \t");
                if (start != std::string::npos)
                {
                    requestedProtocols.push_back(token.substr(start, end - start + 1));
                }
            }
        }

        // Invoke the server's protocol selection callback (if any)
        std::string selectedProtocol;
        if (protocolSelectionCallback)
        {
            if (!protocolSelectionCallback(requestedProtocols, selectedProtocol))
            {
                return sendErrorResponse(400, "Unsupported protocol");
            }
        }
        ...
        {
            ss << "Sec-WebSocket-Protocol: " << selectedProtocol << "\r\n";
        }
        ...
        auto result = WebSocketInitResult(true, 200, "", headers, uri);
