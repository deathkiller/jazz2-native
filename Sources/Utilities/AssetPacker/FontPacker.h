...
namespace Jazz2::AssetPacker
{
	/**
		...
		A font is authored as a grid image - one equally sized cell per character - next to a small file listing
		the character set and how far the pen moves for each. The game instead loads a single file in which every
		glyph has been measured down to the pixels it actually inks, packed against its neighbours and stored as
		palette indices; see @ref Jazz2::UI::FontFormat.
	...
	*/
	...
	public:
		...
		/** @brief Unpacks a packed font back into a grid image and its character list */
