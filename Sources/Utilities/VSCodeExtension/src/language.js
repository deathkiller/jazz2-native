...
const ENTRY_POINTS = [
	...
	},
	{
		...
		doc: 'The console fixed-function implementation of the effect, transpiled to C++ by `--emit-fixed-function`. Never part of the GLSL stages.\n\nEmpty parentheses declare the generic block; `pvr` (Dreamcast), `gx` (Wii/GameCube), `gu` (PlayStation Portable) and `gs` (PlayStation 2) override it for one backend, and a comma-separated target list (`void fixed_function(pvr, gu, gs)`) declares one implementation shared by several. Every target belongs to exactly one block per file.'
	}
];
