...
const STAGE_MACROS = [
	...
	{ name: 'LOW_POWER_GPU', doc: 'Resolved when a stage source is built. `--emit-cg` (PS Vita, sceGxm) and the ESSL 100 (OpenGL|ES 2.0) lowering build with the macro defined — that profile is never a default, it is picked only for hardware that has nothing else. Unlike the other two it says nothing about what the target *can* compile — both run every shader as written — only about how much per-pixel work it can sustain, so gate a cheaper approximation of an expensive path (a nine-tap voronoi of `sin()`-based hashes, say) on it rather than dropping the feature for everyone.' + CONVENTION }
];
