...
//	nuklearShaderProgram_ = std::make_unique<GLShaderProgram>(queryPhase);
...
//	fontTex_ = std::make_unique<GLTexture>(GL_TEXTURE_2D);
...
//
//	nuklearShaderUniforms_ = std::make_unique<GLShaderUniforms>(nuklearShaderProgram_.get());
...
//
//	nuklearShaderAttributes_ = std::make_unique<GLShaderAttributes>(nuklearShaderProgram_.get());
