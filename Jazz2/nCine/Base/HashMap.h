...
namespace nCine
{
	...
	public:
		...
		inline void swap(HashMap& first, HashMap& second)
		{
			...
			std::swap(first.delta1_, second.delta1_);
			std::swap(first.delta2_, second.delta2_);
			std::swap(first.hashes_, second.hashes_);
		...
		inline float loadFactor() const {
			return size_ / static_cast<float>(capacity_);
		}
