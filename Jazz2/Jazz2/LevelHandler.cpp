...
namespace Jazz2
{
		...
		_pressedActions(0)
	{
		...
		_view = std::make_unique<Viewport>(w, h);
		_camera = std::make_unique<Camera>();
		_view->setCamera(_camera.get());
		_rootNode = std::make_unique<SceneNode>();
		_view->setRootNode(_rootNode.get());

		rootView.setNextViewport(_view.get());

		_view->setClearColor(Colorf(0.0f, 0.0f, 0.2f));
		_viewSprite = std::make_unique<Sprite>(&rootNode, _view->texture(), 0.0f, 0.0f);
		_viewSprite->setTexRect(Recti(0, 0, w, h));
		_viewSprite->setPosition(0, 0);
		_viewSprite->setSize((float)res.X, (float)res.Y);
	...
	LevelHandler::~LevelHandler()
	{
		...
		rootView.setNextViewport(nullptr);
	}
	...
	void LevelHandler::OnFrameStart()
	{
	...
	void LevelHandler::OnRootViewportResized(int width, int height)
	{
		...
		_view->resize(w, h);
		_view->texture()->setMagFiltering(SamplerFilter::Nearest);

		_viewSprite->setTexture(_view->texture());
		_viewSprite->setTexRect(Recti(0, 0, w, h));
		_viewSprite->setPosition(0, 0);
